...
PCS_API PcsPanApiRes *pcs_pan_api_res_create() {
  ...
  if (res) memset(res, 0, sizeof(PcsPanApiRes));
  return res;
}

PCS_API void pcs_pan_api_res_destroy(PcsPanApiRes *res) {
  if (res->info_list) pcs_pan_api_res_infolist_destroy(res->info_list);
  pcs_free(res);
}

PCS_API PcsPanApiResInfoList *pcs_pan_api_res_infolist_create() {
  PcsPanApiResInfoList *res =
      (PcsPanApiResInfoList *)pcs_malloc(sizeof(PcsPanApiResInfoList));
  if (res) memset(res, 0, sizeof(PcsPanApiResInfoList));
  return res;
}
