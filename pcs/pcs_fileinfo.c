...
PCS_API PcsFileInfo *pcs_fileinfo_create() {
  ...
  if (res) memset(res, 0, sizeof(PcsFileInfo));
  return res;
}
...
PCS_API PcsSList2 *pcs_fileinfo_add_thumb(PcsFileInfo *fi, const char *name,
                                          const char *url) {
  PcsSList2 *n;
  if (fi->thumbs) {
...
PCS_API PcsFileInfoListItem *pcs_filistitem_create() {
  ...
  if (res) memset(res, 0, sizeof(PcsFileInfoListItem));
  return res;
}
...
PCS_API PcsFileInfoList *pcs_filist_create() {
  ...
  if (res) memset(res, 0, sizeof(PcsFileInfoList));
  return res;
}
...
PCS_API void pcs_filist_remove(PcsFileInfoList *list, PcsFileInfoListItem *item,
                               PcsFileInfoListIterater *iterater) {
  if (item->prev) item->prev->next = item->next;
  if (item->next) item->next->prev = item->prev;
  if (list->link == item) list->link = item->next;
  if (list->link_tail == item) list->link_tail = item->prev;
...
PCS_API void pcs_filist_iterater_init(PcsFileInfoList *list,
                                      PcsFileInfoListIterater *iterater,
                                      PcsBool invert) {
...
PCS_API PcsBool pcs_filist_iterater_next(PcsFileInfoListIterater *iterater) {
  if (!iterater->list->link) return PcsFalse;
  if (iterater->invert) {
    if (!iterater->cursor) {
