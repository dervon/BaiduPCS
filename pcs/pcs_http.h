 ...
 */
typedef size_t (*PcsHttpWriteFunction)(char *ptr, size_t size,
                                       size_t contentlength, void *userdata);
 ...
 */
typedef void (*PcsHttpResponseFunction)(unsigned char *ptr, size_t size,
                                        void *state);
 ...
 * 返回非零值，将导致中断上传或下载
 */
typedef int (*PcsHttpProgressCallback)(void *clientp, double dltotal,
...
/*
 * 返回最后一次请求的
 * HTTP状态码。状态码可参考"http://zh.wikipedia.org/wiki/HTTP%E7%8A%B6%E6%80%81%E7%A0%81"
 */
 ...
 * 例： pcs_http_setopts(handle, PCS_HTTP_OPTION_HTTP_WRITE_FUNCTION, &cb_write,
 * PCS_HTTP_OPTION_HTTP_READ_FUNCTION_DATE, state, PCS_HTTP_OPTION_END);
 */
...
PCS_API char *pcs_http_build_url_v(PcsHttp handle, const char *url,
                                   va_list args);
/*
 * 拼接URL。多参必须全部为 Key-Value的键值对，且类型必须const char
 * *类型。最后一项必须为NULL。 如果值中包含中文或特殊字符，将采用UTF-8来编码。
 *  url  - 基地址
 *  例：
 *     pcs_http_build_url(pcs->http, "http://baidu.com", "key1", "value1",
 * "key2", "value2", NULL); //结果为：http://baidu.com?key1=value1&key2=value2
 *     pcs_http_build_url(pcs->http, "http://baidu.com?s=ab", "key1", "value1",
 * "key2", "&ab", NULL); //结果为：http://baidu.com??s=ab&key1=value1&key2=%26ab
 *     pcs_http_build_url(pcs->http, "http://baidu.com?s=ab&", "key1", "value1",
 * "key2", "&ab", NULL); //结果为：http://baidu.com??s=ab&key1=value1&key2=%26ab
 ...
 * 多参必须全部为 Key-Value的键值对，且类型必须const char
 * *类型。最后一项必须为NULL。 如果值中包含中文或特殊字符，将采用UTF-8来编码。
 *  url  - 基地址
 *  例：
 *     pcs_http_build_post_data(pcs->http, "key1", "value1", "key2", "value2",
 * NULL); //结果为：key1=value1&key2=value2 pcs_http_build_post_data(pcs->http,
 * "key1", "value1", "key2", "&ab", NULL); //结果为：key1=value1&key2=%26ab
 ...
 *   url             服务器地址
 *   follow_location
 * 假如服务器返回跳转到另一个页面的指令时，是否自动跳转过去，如果跳转的话，则返回跳转后页面的内容
 ...
 *   simulate_filename
 * 发送到服务器的文件名。可以指定不同于filename的名字，服务器收到的本地文件名将是simulate_filename而不是filename
 * 例： pcs_http_form_addfile(pcs->http, &form, "file", local_filename,
 * "sample.dat"); //参数名是
 * "file"，文件内容存储在local_filename指定的文件中，服务器接收到的文件名字是"sample.dat"
 ...
 *               The data area pointed at by the pointer ptr may be filled with
 * at most size multiplied with nmemb number of bytes. Your function must return
 ...
 * transfer. userdata    传递到read_func函数第4个参数的值 content_size
 * 期望传递到服务器的内容长度。将会添加到HTTP头中。
 * 添加成功后，返回PcsTrue，否则返回PcsFalse。
 */
...
/*返回下载速度。通过curl_easy_getinfo(http->curl, CURLINFO_SPEED_DOWNLOAD,
 * &downloadSpeed)来获取。*/
