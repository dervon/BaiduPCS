...
static inline void pcs_http_reset_response(struct pcs_http *http) {
  if (http->res_body) pcs_free(http->res_body);
  if (http->res_header) pcs_free(http->res_header);
  if (http->res_encode) pcs_free(http->res_encode);
...
enum HttpMethod { HTTP_METHOD_NONE = 0, HTTP_METHOD_GET, HTTP_METHOD_POST };

static inline void pcs_http_prepare(
    struct pcs_http *http, enum HttpMethod method, const char *url,
    PcsBool follow_location, PcsHttpWriteFunction write_func, void *state,
    curl_off_t max_recv_speed, curl_off_t max_send_speed, long min_recv_speed,
    long min_recv_speed_time, curl_off_t resume_from, curl_off_t max_length) {
  pcs_http_reset_response(http);
  curl_easy_setopt(http->curl, CURLOPT_USERAGENT,
                   http->usage ? http->usage : USAGE);
  ...
  curl_easy_setopt(http->curl, CURLOPT_CONNECTTIMEOUT,
                   (long)http->connect_timeout);
  ...
  if (resume_from || max_length) {
    char *range = pcs_utils_sprintf("%" PRId64 "-%" PRId64, resume_from,
                                    resume_from + max_length - 1);
  ...
  curl_easy_setopt(http->curl, CURLOPT_MAX_RECV_SPEED_LARGE,
                   max_recv_speed ? max_recv_speed : (curl_off_t)0);
  curl_easy_setopt(http->curl, CURLOPT_MAX_SEND_SPEED_LARGE,
                   max_send_speed ? max_send_speed : (curl_off_t)0);
...
static inline PcsBool pcs_http_is_http_redirect_head(struct pcs_http *http,
                                                     char *ptr, size_t size) {
  if ((http->res_type % 2) == 1 && size >= 15 && ptr[0] == 'H' &&
      ptr[1] == 'T' && ptr[2] == 'T' && ptr[3] == 'P' && ptr[4] == '/' &&
      ptr[5] == '1' && ptr[6] == '.' && (ptr[7] == '0' || ptr[7] == '1') &&
...
static inline PcsBool pcs_http_is_http_head(struct pcs_http *http, char *ptr,
                                            size_t size) {
  return (http->res_type % 2) == 0 ? PcsTrue : PcsFalse;
}

static inline int pcs_http_get_content_length_from_header(const char *header,
                                                          int size) {
  int res = -1;
  char *val = NULL;
  const char *p = header, *key = "Content-Length", *end = NULL, *tmp;
...
static inline char *pcs_http_get_charset_from_header(const char *header,
                                                     int size) {
  char *res = NULL;
  const char *p = header, *key = "charset", *end = NULL, *tmp;
...
// static inline char *pcs_http_get_charset_from_body(const char *body, int
// size)
//{
//	char *res = NULL;
...
static inline PcsBool pcs_http_parse_http_head(struct pcs_http *http,
                                               char **ptr, size_t *size,
                                               PcsBool try_get_encode) {
  char *p, *cusor, *end;
  int cnt;
  p = *ptr;
  ...
  if (pcs_http_is_http_head(http, *ptr, *size)) {
    ...
    if (end < http->res_header) end = http->res_header;
    cnt = 0;
    while (cusor > end) {
      if (*cusor == '\n') {
        cnt++;
    ...
    if (cnt == 2) {
      ...
      *size -= cnt;
      ...
      http->res_type++;
      //从头中获取内容长度
      http->res_content_length = pcs_http_get_content_length_from_header(
          http->res_header, http->res_header_size);
      //从头中获取编码
      if (try_get_encode)
        http->res_encode = pcs_http_get_charset_from_header(
            http->res_header, http->res_header_size);
      else
...
static inline char *pcs_http_append_bytes(char *dest, int sz, char *src,
                                          int srcsz) {
  char *p;
...
size_t pcs_http_write(char *ptr, size_t size, size_t nmemb, void *userdata) {
  ...
  if (sz > 0) {
    if (http->res_type == PCS_HTTP_RES_TYPE_NORMAL + 1) {
      http->res_body =
          pcs_http_append_bytes(http->res_body, http->res_body_size, ptr, sz);
      ...
      while (p > ptr) {
        if (*p == 0) {
          if (http->strerror) pcs_free(http->strerror);
          http->strerror =
              pcs_utils_strdup("The response is not the validate text. ");
          return 0;
        }
        p--;
      }
      ...
      return (*http->write_func)(ptr, sz, http->res_content_length,
                                 http->write_data);
    } else
      return 0;
  }
...
static inline char *pcs_http_perform(struct pcs_http *http, const char *url) {
  ...
  if (res != CURLE_OK) {
    if (!http->strerror)
      http->strerror =
          pcs_utils_sprintf("%s: %s", curl_easy_strerror(res),
                            url);  // pcs_utils_strdup(curl_easy_strerror(res));
    return NULL;
  }
  ...
  if (http->response_func)
    (*http->response_func)((unsigned char *)http->res_body,
                           (size_t)http->res_body_size, http->response_data);
...
PCS_API char *pcs_http_build_url_v(PcsHttp handle, const char *url,
                                   va_list args) {
  struct pcs_http *http = (struct pcs_http *)handle;
  char *name, *val, *escapval, *res = NULL, *p;
  int ressz, ampersand = 0;
  ...
  if (*p) {           // have char after '?'
    ...
    if (*p != '&') {  // last char is not '&', so the next param need add '&' in
                      // the front
      ampersand = 1;
    }
  }
...
PCS_API char *pcs_http_get(PcsHttp handle, const char *url,
                           PcsBool follow_location) {
  struct pcs_http *http = (struct pcs_http *)handle;
  pcs_http_prepare(http, HTTP_METHOD_GET, url, follow_location, &pcs_http_write,
                   http, 0, 0, 0, 0, 0, 0);
  return pcs_http_perform(http, url);
}

PCS_API char *pcs_http_get_raw(PcsHttp handle, const char *url,
                               PcsBool follow_location, size_t *sz) {
  struct pcs_http *http = (struct pcs_http *)handle;
  char *data;
  pcs_http_prepare(http, HTTP_METHOD_GET, url, follow_location, &pcs_http_write,
                   http, 0, 0, 0, 0, 0, 0);
...
PCS_API char *pcs_http_post(PcsHttp handle, const char *url, char *post_data,
                            PcsBool follow_location) {
  struct pcs_http *http = (struct pcs_http *)handle;
  pcs_http_prepare(http, HTTP_METHOD_POST, url, follow_location,
                   &pcs_http_write, http, 0, 0, 0, 0, 0, 0);
  if (post_data)
...
PCS_API PcsBool pcs_http_get_download(PcsHttp handle, const char *url,
                                      PcsBool follow_location,
  ...
  struct pcs_http *http = (struct pcs_http *)handle;
  pcs_http_prepare(http, HTTP_METHOD_GET, url, follow_location, &pcs_http_write,
                   http, max_speed, 0, 5 * 1024L,
                   10L, /* 10秒内的平均下载速度低于 5KB/s 的话，则取消下载 */
...
size_t pcs_http_null_write(char *ptr, size_t size, size_t nmemb,
                           void *userdata) {
  return size * nmemb;
}

PCS_API int64_t pcs_http_get_download_filesize(PcsHttp handle, const char *url,
                                               PcsBool follow_location) {
  CURLcode res;
  double downloadFileLenth = 0;
  struct pcs_http *http = (struct pcs_http *)handle;
  pcs_http_prepare(http, HTTP_METHOD_GET, url, follow_location,
                   pcs_http_null_write, NULL, 0, 0, 0, 0, 0, 0);
  curl_easy_setopt(http->curl, CURLOPT_NOBODY, 1L);  //不需要body
  res = curl_easy_perform(http->curl);
  if (res == CURLE_OK) {
    curl_easy_getinfo(http->curl, CURLINFO_CONTENT_LENGTH_DOWNLOAD,
                      &downloadFileLenth);
  } else {
    if (!http->strerror)
      http->strerror = pcs_utils_strdup(curl_easy_strerror(res));
    downloadFileLenth = 0;
  }
  return (int64_t)downloadFileLenth;
}

PCS_API PcsBool pcs_http_form_addfile(PcsHttp handle, PcsHttpForm *post,
                                      const char *param_name,
                                      const char *filename,
                                      const char *simulate_filename) {
  struct pcs_http *http = (struct pcs_http *)handle;
  ...
  escape_simulate_filename =
      simulate_filename ? curl_easy_escape(http->curl, simulate_filename, 0)
                        : NULL;
  if (curl_formadd(&(formpost->formpost), &(formpost->lastptr),
                   CURLFORM_COPYNAME, escape_param_name, CURLFORM_FILE,
                   filename, CURLFORM_FILENAME,
                   escape_simulate_filename ? escape_simulate_filename : " ",
                   CURLFORM_END)) {
    res = PcsFalse;
  }
...
PCS_API PcsBool pcs_http_form_addbufferfile(
    PcsHttp handle, PcsHttpForm *post, const char *param_name,
    const char *simulate_filename,
    size_t (*read_func)(void *ptr, size_t size, size_t nmemb, void *userdata),
    void *userdata, size_t content_size) {
  struct pcs_http *http = (struct pcs_http *)handle;
  struct http_post *formpost = (struct http_post *)(*post);
  char *escape_param_name, *escape_simulate_filename;
  PcsBool res = PcsTrue;
  ...
  escape_simulate_filename =
      simulate_filename ? curl_easy_escape(http->curl, simulate_filename, 0)
                        : NULL;
  if (curl_formadd(&(formpost->formpost), &(formpost->lastptr),
                   CURLFORM_COPYNAME, escape_param_name, CURLFORM_STREAM,
                   userdata, CURLFORM_CONTENTSLENGTH, (long)content_size,
                   CURLFORM_FILENAME,
                   escape_simulate_filename ? escape_simulate_filename : "part",
                   CURLFORM_END)) {
    res = PcsFalse;
  }
  curl_free((void *)escape_param_name);
...
PCS_API PcsBool pcs_http_form_addbuffer(PcsHttp handle, PcsHttpForm *post,
                                        const char *param_name,
  ...
  escape_simulate_filename =
      simulate_filename ? curl_easy_escape(http->curl, simulate_filename, 0)
                        : NULL;
  if (curl_formadd(&(formpost->formpost), &(formpost->lastptr),
                   CURLFORM_COPYNAME, escape_param_name, CURLFORM_BUFFER,
                   escape_simulate_filename ? escape_simulate_filename : "part",
                   CURLFORM_BUFFERPTR, buffer, CURLFORM_BUFFERLENGTH,
                   buffer_size, CURLFORM_END))
    res = PcsFalse;
  curl_free((void *)escape_param_name);
  if (escape_simulate_filename) curl_free((void *)escape_simulate_filename);
  return res;
...
PCS_API char *pcs_post_httpform(PcsHttp handle, const char *url,
                                PcsHttpForm data, curl_off_t max_speed,
                                PcsBool follow_location) {
  struct pcs_http *http = (struct pcs_http *)handle;
  struct http_post *formpost = (struct http_post *)data;
  char *rc;
  pcs_http_prepare(http, HTTP_METHOD_POST, url, follow_location,
                   &pcs_http_write, http, 0, max_speed, 0, 0, 0, 0);
  if (data) {
...
PCS_API const char *pcs_http_rawdata(PcsHttp handle, int *size,
                                     const char **encode) {
  struct pcs_http *http = (struct pcs_http *)handle;
...
PCS_API double pcs_http_speed_download(PcsHttp handle) {
  ...
  struct pcs_http *http = (struct pcs_http *)handle;
  CURLcode re = curl_easy_getinfo(http->curl, CURLINFO_SPEED_DOWNLOAD,
                                  &downloadSpeed);  // 获取下载速度
  if (re == CURLE_OK)
