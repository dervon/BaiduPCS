...
typedef void (*pcs_mem_leak_print_fun)(void *ptr, const char *filename,
                                       int line);
...
PCS_API void *pcs_mem_malloc_raw(size_t size) { return malloc(size); }

PCS_API void pcs_mem_free_raw(void *ptr) { free(ptr); }
