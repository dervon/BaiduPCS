...
PCS_API char *pcs_utils_strcat(char *dst, const char *src, int src_len,
                               int free_dst) {
  char *res = 0;
  int dst_len;

  if (!src) return dst;
  if (src_len == -1) src_len = strlen(src);
...
PCS_API char *pcs_utils_readable_size(double size /*in bytes*/, char *buf,
                                      int buf_size, char *sp) {
  int i = 0;
...
PCS_API char *pcs_utils_readable_left_time(int64_t second, char *buf,
                                           int buf_size, char *sp) {
...
PCS_API const unsigned char *pcs_md5_bytes_raw(const unsigned char *bytes,
                                               int len) {
  static unsigned char md[16];
  MD5(bytes, len, md);
  return md;
 ...
 * 例：0xF1E2D3C4 填充后 buf[0] = 0xF1, buf[1] = 0xE2, buf[2] = 0xD3, buf[3] =
 * 0xC4.buf中其他项无改动
 */
...
/* For C# invoke */
PCS_API int pcs_strlen(const char *s) { return strlen(s); }
