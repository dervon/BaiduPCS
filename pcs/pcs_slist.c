...
PCS_API PcsSList *pcs_slist_create() {
  ...
  if (res) memset(res, 0, sizeof(PcsSList));
  return res;
}
...
PCS_API void pcs_slist_destroy(PcsSList *slist) {
  ...
  while (p) {
    p2 = p;
    p = p->next;
    if (p2->string) pcs_free(p2->string);
    pcs_free(p2);
  }
}
...
PCS_API PcsSList2 *pcs_slist2_create() {
  ...
  if (res) memset(res, 0, sizeof(PcsSList2));
  return res;
}

PCS_API PcsSList2 *pcs_slist2_create_ex(const char *src1, int len1,
...
PCS_API void pcs_slist2_destroy(PcsSList2 *slist) {
  ...
  while (p) {
    p2 = p;
    p = p->next;
    if (p2->string1) pcs_free(p2->string1);
    if (p2->string2) pcs_free(p2->string2);
    pcs_free(p2);
  }
}

PCS_API PcsSList2 *pcs_slist2_add_ex(PcsSList2 *slist, const char *src1,
                                     int len1, const char *src2, int len2) {
...
PCS_API PcsSList2 *pcs_slist2_clone(PcsSList2 *slist) {
  ...
  while (p) {
    if (n) {
      n = pcs_slist2_add_ex(n, p->string1, strlen(p->string1), p->string2,
                            strlen(p->string2));
    } else {
      n = newlist = pcs_slist2_create_ex(p->string1, strlen(p->string1),
                                         p->string2, strlen(p->string2));
    }
    if (!n) {
