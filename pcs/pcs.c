...
#define PCS_SKIP_SPACE(p)                                                \
  while ((*p) && (*p == ' ' || *p == '\f' || *p == '\n' || *p == '\r' || \
                  *p == '\t' || *p == '\v'))                             \
  p++

#define PCS_IS_TOKEN_CHAR(ch) \
  ((ch) != '&' && (ch) != '?' && (ch) != '#' && (ch) > 32 && (ch) < 127)
...
#define URL_AUTHWIDGET_VERIFY                           \
  "http://passport.baidu.com/v2/sapi/authwidgetverify?" \
  "authtoken=%s"                                        \
  "&type=%s"                                            \
  "&jsonp=1"                                            \
  "&apiver=v3"                                          \
...
PcsRes pcs_get_captcha(Pcs handle, const char *code_string, char *captcha,
  ...
  if (!pcs->captcha_func) {
    pcs_set_errmsg(handle,
                   "No captch function, please regist the function by call "
                   "pcs_setopt(handle, PCS_OPTION_CAPTCHA_FUNCTION, pFun).");
  ...
  if (!img) {
    pcs_set_errmsg(
        handle,
        "Can't get captch image from the server. url: %s. Error message: %s",
        url, pcs_http_strerror(pcs->http));
    pcs_free(url);
    return PCS_NETWORK_ERROR;
  }
  if (!(*pcs->captcha_func)((unsigned char *)img, imgsz, captcha, captchaSize,
                            pcs->captcha_data)) {
    pcs_set_errmsg(
        handle, "Can't get captch from the captcha, which is regist by user.");
    pcs_free(url);
...
PcsRes pcs_input(Pcs handle, const char *tips, char *value, size_t valueSize) {
  ...
  if (!pcs->input_func) {
    pcs_set_errmsg(handle,
                   "No input function, please regist the function by call "
                   "pcs_setopt(handle, PCS_OPTION_INPUT_FUNCTION, pFun).");
...
/*从html中解析出类似于 FileUtils.bdstoken="****"
 * 的值。此例中，key传入"FileUtils.bdstoken"，将返回"****"值*/
static char *pcs_get_value_by_key(const char *html, const char *key) {
  char *val = NULL;
  const char *p = html, *end = NULL, *tmp;
  int i = strlen(key);  // \f\n\r\t\v
  while (*p) {
...
/*从URL地址的QueryString中解析出类似于 &error=123&a=
 * 的值。此例中，key传入"&error"，将返回123*/
static char *pcs_get_embed_query_int_value_by_key(const char *html,
                                                  const char *key) {
  char *val = NULL;
  const char *p = html, *end = NULL, *tmp;
  int i = strlen(key);  // \f\n\r\t\v
...
/*从URL地址的QueryString中解析出类似于 &token=abc_ef-g&a=
 * 的值。此例中，key传入"&token"，将返回abc_ef-g*/
static char *pcs_get_embed_query_token_by_key(const char *html,
                                              const char *key) {
  char *val = NULL;
  const char *p = html, *end = NULL, *tmp;
  int i = strlen(key);  // \f\n\r\t\v
...
static char *pcs_build_pan_api_url_v(Pcs handle, const char *action,
                                     va_list args) {
  struct pcs *pcs = (struct pcs *)handle;
  char *baseurl, *url;
  ...
  baseurl = pcs_http_build_url(
      pcs->http, baseurl, "channel", "chunlei", "clienttype", "0", "web", "1",
      "t", pcs_js_timestr(), "bdstoken", pcs->bdstoken, NULL);
...
/*传入action，即参数，拼接出最终字符串。参数以(key,
value)对的形式传入，最后传入一个NULL表示终止 例：pcs_build_pan_api_url(handle,
"list", k1, v1, k2, v2, NULL);
...
static PcsFileInfo *pcs_parse_fileinfo(cJSON *item) {
  ...
  if (val) fi->path = pcs_utils_strdup(val->valuestring);

  val = cJSON_GetObjectItem(item, "server_filename");
  if (val) fi->server_filename = pcs_utils_strdup(val->valuestring);

  val = cJSON_GetObjectItem(item, "mtime");
  if (val) fi->server_mtime = (time_t)val->valuedouble;

  val = cJSON_GetObjectItem(item, "ctime");
  if (val) fi->server_ctime = (time_t)val->valuedouble;
  ...
  if (val) fi->local_mtime = (time_t)val->valuedouble;

  val = cJSON_GetObjectItem(item, "local_ctime");
  if (val) fi->local_ctime = (time_t)val->valuedouble;

  val = cJSON_GetObjectItem(item, "isdir");
  if (val) fi->isdir = val->valueint ? PcsTrue : PcsFalse;

  val = cJSON_GetObjectItem(item, "category");
  if (val) fi->category = val->valueint;

  val = cJSON_GetObjectItem(item, "size");
  if (val) fi->size = (int64_t)val->valuedouble;

  val = cJSON_GetObjectItem(item, "dir_empty");
  if (val) fi->dir_empty = val->valueint ? PcsTrue : PcsFalse;

  val = cJSON_GetObjectItem(item, "empty");
  if (val) fi->empty = val->valueint ? PcsTrue : PcsFalse;

  val = cJSON_GetObjectItem(item, "ifhassubdir");
  if (val) fi->ifhassubdir = val->valueint ? PcsTrue : PcsFalse;

  val = cJSON_GetObjectItem(item, "md5");
  if (val) fi->md5 = pcs_utils_strdup(val->valuestring);

  val = cJSON_GetObjectItem(item, "dlink");
  if (val) fi->dlink = pcs_utils_strdup(val->valuestring);
...
static char *pcs_build_filelist_2(Pcs handle, PcsSList2 *slist, int *count) {
  ...
  while (item) {
    while (*p) p++;
    sprintf(p, "{\"path\":\"%s\",\"newname\":\"%s\"},", item->string1,
            item->string2);
    p += 25;
    item = item->next;
  }
...
*/
static PcsPanApiRes *pcs_pan_api_filemanager(Pcs handle, const char *opera,
                                             const char *filelist,
                                             int file_count) {
  struct pcs *pcs = (struct pcs *)handle;
  ...
  url = pcs_build_pan_api_url(handle, "filemanager", "opera", opera, NULL);
  if (!url) {
  ...
  json = cJSON_Parse(html);
  if (!json) {
    pcs_set_errmsg(handle, "Can't parse the response as object. Response: %s",
                   html);
    return NULL;
  }
  item = cJSON_GetObjectItem(json, "errno");
  ...
  for (i = 0; i < cnt; i++) {
    ...
    val = cJSON_GetObjectItem(item, "path");
    if (val) ri->info.path = pcs_utils_strdup(val->valuestring);
    ...
    if (is_move || strcmp(opera, "copy") == 0) {
      if (res->error == 12) {
        if (!err_no_space && !err_src_file_not_exist && !err_target_not_exist) {
          pcs_set_errmsg(handle, "%s%s", is_move ? "移动" : "复制",
                         "失败，请稍候重试");
        } else {
      ...
      if (res->error == -8 || res->error == -9 ||
          (cnt > 0 && (res->info_list->info.error == -8 ||
                       res->info_list->info.error == -9)))
...
static PcsFileInfo *pcs_upload_form(Pcs handle, const char *path,
                                    PcsHttpForm form, curl_off_t max_speed,
                                    const char *ondup) {
  struct pcs *pcs = (struct pcs *)handle;
  char *url, *html, *dir = pcs_utils_basedir(path),
                    *filename = pcs_utils_filename(path);
  cJSON *json, *item;
  PcsFileInfo *meta;

  url = pcs_http_build_url(pcs->http, URL_PCS_REST, "method", "upload",
                           "app_id", "250528", "ondup", ondup, "dir", dir,
                           "filename", filename, "BDUSS", pcs->bduss, NULL);
  pcs_free(dir);
  pcs_free(filename);
  if (!url) {
    pcs_set_errmsg(handle, "Can't build the url.");
  ...
  //"{\"error_code\":31062,\"error_msg\":\"file name is
  // invalid\",\"request_id\":3071564675}
  item = cJSON_GetObjectItem(json, "error_code");
  if (item) {
    int error_code = item->valueint;
    const char *error_msg = NULL;
    item = cJSON_GetObjectItem(json, "error_msg");
    if (item) error_msg = item->valuestring;
    pcs_set_errmsg(
        handle,
        "Can't upload file. error_code: %d. error_msg: %s. raw response: %s",
        error_code, error_msg, html);
    cJSON_Delete(json);
    return NULL;
  }
...
static PcsFileInfo *pcs_upload_slice_form(Pcs handle, PcsHttpForm form,
  ...
  url = pcs_http_build_url(pcs->http, URL_PCS_REST, "method", "upload", "type",
                           "tmpfile", "app_id", "250528", "BDUSS", pcs->bduss,
                           NULL);
  if (!url) {
    pcs_set_errmsg(handle, "Can't build the url.");
    return NULL;
...
//	int pemLen = base64Len + base64Len / 64; // PEM adds a newline every 64
// bytes 	char* base64 = (char *)pcs_malloc(pemLen + 1); 	int result;
//	EVP_EncodeInit(&ctx);
//	EVP_EncodeUpdate(&ctx, (unsigned char *)base64, &result, (unsigned char
//*)inputBuffer, inputLen); 	EVP_EncodeFinal(&ctx, (unsigned char
//*)&base64[result], &result); 	return remove_enter(base64);
//}
...
//== '=' || *p == '/' || *p == '\\' || *p == ' ' || *p == '\f' || *p == '\r' ||
//*p == '\n' || *p == '\t') 	while (*p) { 		if (IS_SYMBOL(p))
//newLen += 3; 		else 			newLen++; 		p++;
//	}
//	p = buf;
...
//	if (RSA_public_encrypt(rsa_len, (unsigned char *)str, (unsigned
// char*)p_en, rsa, RSA_NO_PADDING)<0){ 		return NULL;
//	}
//	RSA_free(rsa);
//
...
PCS_API void pcs_destroy(Pcs handle) {
  struct pcs *pcs = (struct pcs *)handle;
  if (pcs->http) pcs_http_destroy(pcs->http);
  ...
  if (pcs->errmsg) pcs_free(pcs->errmsg);
...
PCS_API void pcs_clone_userinfo(Pcs dst, Pcs src) {
  ...
  if (pcs_src->bduss) pcs_dst->bduss = pcs_utils_strdup(pcs_src->bduss);

  if (pcs_dst->sysUID) {
    pcs_free(pcs_dst->sysUID);
  }
  if (pcs_src->sysUID) pcs_dst->sysUID = pcs_utils_strdup(pcs_src->sysUID);
  ...
  if (pcs_src->key) pcs_dst->key = pcs_utils_strdup(pcs_src->key);
}
...
PCS_API PcsRes pcs_setopt(Pcs handle, PcsOption opt, void *value) {
  ...
  switch (opt) {
    ...
    case PCS_OPTION_HTTP_RESPONSE_FUNCTION_DATE:
      pcs_http_setopt(pcs->http, PCS_HTTP_OPTION_HTTP_RESPONSE_FUNCTION_DATE,
                      value);
      break;
...
static PcsRes pcs_prelogin(__in Pcs handle, __out char **token,
                           __out char **code_string) {
  struct pcs *pcs = (struct pcs *)handle;
  char *html, *url, *p;
  const char *errmsg;
  ...
  url = pcs_utils_sprintf(URL_PASSPORT_API
                          "getapi"
                          "&tpl=netdisk"
                          "&apiver=v3"
                          "&tt=%s"
...
static char *pcs_append_substr(char *dst, char **src, const char *begin,
                               const char *end) {
  char *first = strstr(*src, begin), *last;
  if (first) {
    first += strlen(begin);
...
static PcsRes pcs_dologin(__in Pcs handle, __in int64_t starttime,
                          __in const char *code_string, __in const char *token,
                          __out int *errorno, __out char **out_html) {
  struct pcs *pcs = (struct pcs *)handle;
  PcsRes res;
  ...
  post_data = pcs_http_build_post_data(
      pcs->http, "staticpage",
      "http://pan.baidu.com/res/static/thirdparty/pass_v3_jump.html", "charset",
      "utf-8", "token", token, "tpl", "netdisk", "subpro", "", "apiver", "v3",
      "tt", pcs_js_timestr(), "codestring", code_string, "safeflg", "0", "u",
      "http://pan.baidu.com/", "isPhone", "", "quick_user", "0", "logintype",
      "basicLogin", "logLoginType", "pc_loginBasic", "idc", "", "loginmerge",
      "true", "username", pcs->username, "password", pcs->password,
      "verifycode", captch, "mem_pass", "on", "rsakey", "", "crypttype", "",
      "ppui_logintime", "2602", "dv", dv, "callback", "parent.bd__pcbs__msdlhs",
      NULL);
  ...
  if (!html) {
    ...
    if (!errorStr)
      errorStr = pcs_get_embed_query_int_value_by_key(html, "err_no");
    if (!errorStr) {
      pcs_set_errmsg(
          handle, "Can't read the error from the response. Response: %s", html);
      return PCS_NETWORK_ERROR;
    }
    *errorno = atoi(errorStr);
    pcs_free(errorStr);

    if (out_html != NULL) *out_html = pcs_utils_strdup(html);
...
static PcsRes pcs_call_authwidgetverify(
    __in Pcs handle, __in const char *authtoken, __in const char *vtype,
    __in const char *action, __in const char *vcode, __out int *errorno,
    __out char **authsid, __out PcsSList2 **pdata, __out char **pmsg) {
  struct pcs *pcs = (struct pcs *)handle;
  char *url, *html;
  const char *errmsg;
  cJSON *json, *root, *item;
  ...
  url =
      pcs_utils_sprintf(URL_AUTHWIDGET_VERIFY, authtoken, vtype, action, vcode);
  html = pcs_http_get(pcs->http, url, PcsTrue);
  pcs_free(url);
  if (!html) {
    errmsg = pcs_http_strerror(pcs->http);
  ...
  *errorno = atoi(item->valuestring);
  ...
  if (pdata) {
    ...
    if (root && root->type == cJSON_Object) {
      static const char *verfify_items[] = {"softtoken", "mobile", "email",
                                            "password"};
        ...
        if (item && item->type == cJSON_String &&
          ...
          if (data == NULL)
            data = pcs_slist2_create_ex(
                verfify_items[i], strlen(verfify_items[i]), item->valuestring,
                strlen(item->valuestring));
          else
            pcs_slist2_add_ex(data, verfify_items[i], strlen(verfify_items[i]),
...
static PcsRes pcs_call_auththaw(__in Pcs handle, __in const char *gotourl,
...
static char *build_verify_api_list_string(__in PcsSList2 *list, __out char *buf,
                                          __in int buf_len) {
  int len, id;
  PcsSList2 *api;
  ...
  id = 1;
  while (api) {
    snprintf(buf + len, buf_len - len, "  %d) %s \t%s\n", id, api->string1,
             api->string2);
    api = api->next;
    id++;
    len = strlen(buf);
...
static PcsSList2 *get_verify_api_by_index(__in PcsSList2 *list,
                                          __in int index) {
  PcsSList2 *api = list;
  int i = 0;
  while (api) {
...
static PcsRes pcs_do_auth_verify(__in Pcs handle, __in const char *authtoken,
                                 __out int *res_error, __out char **authsid) {
  struct pcs *pcs = (struct pcs *)handle;
  PcsRes res;
  int error;
  ...
  res = pcs_call_authwidgetverify(handle, authtoken, "", "getapi", "", &error,
                                  NULL, &verify_api_list, &msg);
  *res_error = error;
  if (res != PCS_OK) {
    return res;
  ...
  res = pcs_call_authwidgetverify(handle, authtoken, vm, "send", "", &error,
                                  NULL, NULL, &msg);
  *res_error = error;
  if (res != PCS_OK) {
    pcs_slist2_destroy(verify_api_list);
    ...
    if (msg)
      pcs_set_errmsg(handle, "Can't send verify code to \"%s\": %s",
                     selected_item->string2, msg);
    else
      pcs_set_errmsg(handle, "Can't send verify code to \"%s\".",
                     selected_item->string2);
    pcs_free(msg);
    pcs_slist2_destroy(verify_api_list);
    return PCS_FAIL;
  }
  ...
  if (strncmp(vm, "mobile", sizeof("mobile") - 1) == 0)
    snprintf(tips, sizeof(tips) - 1, "Please input %s password: ", "sms");
  else if (strncmp(vm, "email", sizeof("email") - 1) == 0)
    snprintf(tips, sizeof(tips) - 1, "Please input %s password: ", "email");
...
try_login:
  ...
  if (retry_times < 3 &&
      (error == 3 || error == 6 || error == 257 || error == 200010)) {
    ...
    if (!code_string) {
      pcs_set_errmsg(
          handle, "Can't read the codestring from the response. Response: %s",
          html);
      pcs_free(token);
      pcs_free(code_string);
      pcs_free(html);
    ...
    retry_times++;
    goto try_login;
  } else if (error == 0 || error == 18 || error == 400032 || error == 400034 ||
             error == 400037 || error == 400401) {
...
PCS_API PcsRes pcs_quota(Pcs handle, int64_t *quota, int64_t *used) {
  ...
  json = cJSON_Parse(html);
  if (!json) {
    pcs_set_errmsg(handle, "Can't parse the response as json. Response: %s",
                   html);
    return PCS_WRONG_RESPONSE;
  }
  ...
  if (quota) *quota = (int64_t)item->valuedouble;
  ...
  if (used) *used = (int64_t)item->valuedouble;
...
PCS_API PcsRes pcs_mkdir(Pcs handle, const char *path) {
  ...
  postdata =
      pcs_http_build_post_data(pcs->http, "path", path, "isdir", "1", "size",
                               "", "block_list", "[]", "method", "post", NULL);
  if (!postdata) {
    pcs_set_errmsg(handle, "Can't build the post data.");
    pcs_free(url);
    return PCS_BUILD_POST_DATA;
...
PCS_API PcsFileInfoList *pcs_list(Pcs handle, const char *dir, int pageindex,
                                  int pagesize, const char *order,
                                  PcsBool desc) {
  struct pcs *pcs = (struct pcs *)handle;
  char *page, *pagenum;
  PcsFileInfoList *filist = NULL;
  ...
  filist = pcs_pan_api_1(handle, "list", "_", pcs_js_timestr(), "dir", dir,
                         "page", page, "num", pagenum, "order", order,
                         desc ? "desc" : "", desc ? "1" : "0", NULL);
  pcs_free(page);
  pcs_free(pagenum);
  return filist;
}

PCS_API PcsFileInfoList *pcs_search(Pcs handle, const char *dir,
                                    const char *key, PcsBool recursion) {
  struct pcs *pcs = (struct pcs *)handle;
  PcsFileInfoList *filist = NULL;
  pcs_clear_errmsg(handle);
  filist = pcs_pan_api_1(handle, "search", "dir", dir, "key", key,
                         recursion ? "recursion" : "", NULL, NULL);
  return filist;
}
...
static PcsRes pcs_download_normal(Pcs handle, const char *path,
                                  PcsHttpWriteFunction write, void *write_state,
                                  curl_off_t max_speed, curl_off_t resume_from,
                                  curl_off_t max_length) {
  struct pcs *pcs = (struct pcs *)handle;
  char *url;
  const char *errmsg;
  ...
  pcs_http_setopts(pcs->http, PCS_HTTP_OPTION_HTTP_WRITE_FUNCTION, write,
                   PCS_HTTP_OPTION_HTTP_WRITE_FUNCTION_DATE, write_state,
                   PCS_HTTP_OPTION_END);
  url = pcs_http_build_url(pcs->http, URL_PCS_REST, "method", "download",
                           "app_id", "250528", "path", path, NULL);
  if (!url) {
    pcs_set_errmsg(handle, "Can't build the url.");
    return PCS_BUILD_URL;
  }
  ...
  if (errmsg)
    ...
    pcs_set_errmsg(handle, "Can't download the file: %s",
                   pcs_http_strerror(pcs->http));
  return PCS_FAIL;
}

PCS_API PcsRes pcs_download(Pcs handle, const char *path, curl_off_t max_speed,
                            curl_off_t resume_from, curl_off_t max_length) {
  struct pcs *pcs = (struct pcs *)handle;
  return pcs_download_normal(handle, path, pcs->download_func,
                             pcs->download_data, max_speed, resume_from,
                             max_length);
}
...
static size_t pcs_cat_write_func(char *ptr, size_t size, size_t contentlength,
                                 void *userdata) {
...
PCS_API PcsFileInfo *pcs_upload_buffer(Pcs handle, const char *path,
                                       PcsBool overwrite, const char *buffer,
                                       size_t buffer_size,
                                       curl_off_t max_speed) {
  struct pcs *pcs = (struct pcs *)handle;
  char *filename;
  ...
  pcs_clear_errmsg(handle);
  filename = pcs_utils_filename(path);
  if (pcs_http_form_addbuffer(pcs->http, &form, "file", buf, (long)sz,
                              filename) != PcsTrue) {
    pcs_set_errmsg(handle, "Can't build the post data.");
    pcs_free(filename);
  ...
  pcs_free(filename);
  meta = pcs_upload_form(handle, path, form, max_speed,
                         overwrite ? "overwrite" : "newcopy");
  pcs_http_form_destroy(pcs->http, form);
  if (buf != buffer) pcs_free(buf);
  return meta;
}

PCS_API PcsFileInfo *pcs_upload_slice(Pcs handle, const char *buffer,
                                      size_t buffer_size,
                                      curl_off_t max_speed) {
  struct pcs *pcs = (struct pcs *)handle;
  PcsHttpForm form = NULL;
  ...
  pcs_clear_errmsg(handle);
  if (pcs_http_form_addbuffer(pcs->http, &form, "file", buf, (long)sz, NULL) !=
      PcsTrue) {
    pcs_set_errmsg(handle, "Can't build the post data.");
    if (buf != buffer) pcs_free(buf);
    return NULL;
...
PCS_API PcsFileInfo *pcs_upload_slicefile(
    Pcs handle,
    size_t (*read_func)(void *ptr, size_t size, size_t nmemb, void *userdata),
    void *userdata, size_t content_size, curl_off_t max_speed) {
  struct pcs *pcs = (struct pcs *)handle;
  PcsHttpForm form = NULL;
  PcsFileInfo *meta;

  pcs_clear_errmsg(handle);
  if (pcs_http_form_addbufferfile(pcs->http, &form, "file", NULL, read_func,
                                  userdata, content_size) != PcsTrue) {
    pcs_set_errmsg(handle, "Can't build the post data.");
    return NULL;
  }
  meta = pcs_upload_slice_form(handle, form, max_speed);
...
PCS_API PcsFileInfo *pcs_create_superfile(Pcs handle, const char *path,
                                          PcsBool overwrite,
                                          PcsSList *block_list) {
  struct pcs *pcs = (struct pcs *)handle;
  ...
  url = pcs_http_build_url(pcs->http, URL_PCS_REST, "method", "createsuperfile",
                           "app_id", "250528", "path", path, "ondup",
                           overwrite ? "overwrite" : "newcopy", "BDUSS",
                           pcs->bduss, NULL);
  if (!url) {
    pcs_set_errmsg(handle, "Can't build the url.");
    return NULL;
  ...
  postdata =
      pcs_http_build_post_data(pcs->http, "param", block_list_data, NULL);
  pcs_free(block_list_data);
  if (!postdata) {
    pcs_set_errmsg(handle, "Can't build the post data.");
    pcs_free(url);
...
PCS_API PcsFileInfo *pcs_upload(Pcs handle, const char *path, PcsBool overwrite,
  ...
  pcs_clear_errmsg(handle);
  filename = pcs_utils_filename(path);
  if (pcs_http_form_addfile(pcs->http, &form, "file", local_filename,
                            filename) != PcsTrue) {
    pcs_set_errmsg(handle, "Can't build the post data.");
    pcs_free(filename);
    return NULL;
...
PCS_API PcsFileInfo *pcs_upload_s(
    Pcs handle, const char *to_path, PcsBool overwrite,
    size_t (*read_func)(void *ptr, size_t size, size_t nmemb, void *userdata),
    void *userdata, size_t content_size, curl_off_t max_speed) {
  struct pcs *pcs = (struct pcs *)handle;
  char *filename;
  ...
  meta = pcs_upload_form(handle, to_path, form, max_speed,
                         overwrite ? "overwrite" : "newcopy");
  pcs_http_form_destroy(pcs->http, form);
  return meta;
}
...
PCS_API PcsBool pcs_md5_s(Pcs handle,
  ...
  while (((sz = read_func(buf, 1, PCS_BUFFER_SIZE, userdata)) > 0) &&
         (sz != CURL_READFUNC_ABORT) && (sz != CURL_READFUNC_PAUSE)) {
    MD5_Update(&md5, buf, sz);
  }
...
PCS_API PcsBool pcs_md5_file_slice(Pcs handle, const char *path, int64_t offset,
                                   int64_t length, char *md5_buf) {
  MD5_CTX md5;
  FILE *pf;
  size_t sz;
  int64_t hashed_sz = 0;
  ...
  MD5_Init(&md5);
  while ((hashed_sz < length) &&
         ((sz = fread(buf, 1, PCS_BUFFER_SIZE, pf)) > 0)) {
...
PCS_API PcsFileInfo *pcs_rapid_upload(Pcs handle, const char *path,
                                      PcsBool overwrite,
                                      const char *local_filename,
                                      char *out_content_md5,
                                      char *out_slice_md5) {
  struct pcs *pcs = (struct pcs *)handle;
  ...
  if (content_length <= PCS_RAPIDUPLOAD_THRESHOLD) {
    pcs_set_errmsg(
        handle,
        "The file size is not satisfied, the file must be great than %d.",
  ...
  if (out_slice_md5 != NULL && strnlen(out_slice_md5, 33) == 32) {
    ...
    if (!pcs_md5_file_slice(handle, local_filename, 0,
                            PCS_RAPIDUPLOAD_THRESHOLD, slice_md5)) {
      return NULL;
    }
    if (out_slice_md5) strcpy(out_slice_md5, slice_md5);
  }

  return pcs_rapid_upload_r(handle, path, overwrite, content_length,
                            content_md5, slice_md5);
}

PCS_API PcsFileInfo *pcs_rapid_upload_r(Pcs handle, const char *path,
                                        PcsBool overwrite,
                                        int64_t content_length,
                                        const char *content_md5,
                                        const char *slice_md5) {
  struct pcs *pcs = (struct pcs *)handle;
  ...
  url = pcs_http_build_url(
      pcs->http, URL_PCS_REST, "method", "rapidupload", "app_id", "250528",
      "ondup", overwrite ? "overwrite" : "newcopy", "dir", dir, "filename",
      filename, "content-length", content_length_str, "content-md5",
      content_md5, "slice-md5", slice_md5, "path", path, "BDUSS", pcs->bduss,
      "bdstoken", pcs->bdstoken, NULL);
  if (!url) {
    pcs_set_errmsg(handle, "Can't build the url.");
...
PCS_API const char *pcs_req_rawdata(Pcs handle, int *size,
                                    const char **encode) {
  struct pcs *pcs = (struct pcs *)handle;
