...
static HashtableNode *node_create(const char *key, int key_size, void *value,
                                  unsigned int hashA, unsigned int hashB) {
  HashtableNode *node;
...
static void node_destroy(HashtableNode *node, void (*free_value)(void *)) {
  ...
  while (cusor) {
    tmp = cusor;
    cusor = cusor->next;
    if (tmp->key) pcs_free(tmp->key);
    if (free_value && tmp->value) (*free_value)(tmp->value);
    pcs_free(tmp);
  }
}

static int table_add_item(HashtableNode **table, int real_capacity,
                          const char *key, int key_size, void *value,
                          int ignore_case) {
...
static int table_set_item(HashtableNode **table, int real_capacity,
                          const char *key, int key_size, void *value,
                          int ignore_case, int *pHasOld, void **pOldVal) {
  //不同的字符串三次hash还会碰撞的几率无限接近于不可能
  unsigned int nHash, nHashA, nHashB;
  unsigned int pos;
  HashtableNode *last = NULL, *p;
...
static HashtableNode *table_get_item(HashtableNode **table, int real_capacity,
...
static HashtableNode *table_remove_item(HashtableNode **table,
                                        int real_capacity, const char *key,
                                        int key_size, int ignore_case) {
  unsigned int nHash, nHashA, nHashB;
  unsigned int pos;
...
static void table_clear(HashtableNode **table, int real_capacity,
...
static void table_destroy(HashtableNode **table, int real_capacity,
                          void (*free_value)(void *)) {
  table_clear(table, real_capacity, free_value);
  pcs_free(table);
}

Hashtable *ht_create(int capacity, int ignore_case,
                     void (*free_value)(void *)) {
  Hashtable *ht;
  ...
  ht->table =
      (HashtableNode **)pcs_malloc(ht->real_capacity * sizeof(HashtableNode *));
  if (!ht->table) {
    pcs_free(ht);
    return NULL;
  }
...
int ht_expand(Hashtable *ht, int capacity) {
  ...
  for (i = 0; i < ht->real_capacity; i++) {
    node = ht->table[i];
    while (node) {
      if (table_add_item(table, real_capacity, node->key, -1, node->value,
                         ht->ignore_case)) {
...
int ht_add(Hashtable *ht, const char *key, int key_size, void *value) {
  if (ht->count >= ht->capacity) {
    if (ht_expand(ht, ht->count * 2)) return -1;
  }
  if (table_add_item(ht->table, ht->real_capacity, key, key_size, value,
                     ht->ignore_case))
    return -1;
  ht->count++;
  return 0;
...
int ht_set(Hashtable *ht, const char *key, int key_size, void *value,
           void **pOldVal) {
  void *oldVal = NULL;
  int rc, pHasOld;
  rc = table_set_item(ht->table, ht->real_capacity, key, key_size, value,
                      ht->ignore_case, &pHasOld, pOldVal ? pOldVal : &oldVal);
  if (oldVal && ht->free_value)
    (*ht->free_value)(oldVal); /*如果pOldVal传入NULL，且有旧值时，自动释放*/
  if (!pHasOld) ht->count++;
  return rc;
}
...
int ht_remove(Hashtable *ht, const char *key, int key_size, void **pVal) {
  HashtableNode *p;
  p = table_remove_item(ht->table, ht->real_capacity, key, key_size,
                        ht->ignore_case);
  if (p) {
...
void *ht_get(Hashtable *ht, const char *key, int key_size) {
  HashtableNode *p;
  p = table_get_item(ht->table, ht->real_capacity, key, key_size,
                     ht->ignore_case);
  if (!p) return NULL;
  return p->value;
}
...
HashtableNode *ht_get_node(Hashtable *ht, const char *key, int key_size) {
  return table_get_item(ht->table, ht->real_capacity, key, key_size,
                        ht->ignore_case);
}
...
void ht_it_destroy(HashtableIterater *iterater) { pcs_free(iterater); }

void ht_it_reset(HashtableIterater *iterater) { iterater->index = -1; }

int ht_it_next(HashtableIterater *iterater) {
  if (iterater->ht->count == 0) return 0;
  if (iterater->p) {
    iterater->p = iterater->p->next;
    if (iterater->p) return 1;
  }
  if (iterater->index >= iterater->ht->real_capacity) return 0;
  iterater->index++;
...
void *ht_it_current(HashtableIterater *iterater) {
  if (iterater->p) return iterater->p->value;
  return NULL;
}
