...
#define THREAD_STATE_MAGIC \
  (((int)'T' << 24) | ((int)'S' << 16) | ((int)'H' << 8) | ((int)'T'))
...
int u8_mbs_toutf8(char *dest, int sz, const char *src, int srcsz) {
  ...
  err =
      MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, src, srcsz, unicode, wchars);
  if (err != wchars) {
    fprintf(stderr, "Unicode translation error %d\n", GetLastError());
    return -1;
  }
...
int u8_tombs(char *dest, int sz, const char *src, int srcsz) {
  ...
  err = WideCharToMultiByte(GetConsoleCP(), WC_COMPOSITECHECK, unicode,
                            unicode_size, dest, sz, NULL, NULL);
  if (err < 1) {
    fprintf(stderr, "Unicode translation error %d\n", GetLastError());
    return -1;
  }
...
int truncate(const char *file, int64_t size) {
  HANDLE hFile;
  hFile = CreateFile(file, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING,
...
#else
/*判断当前操作系统编码是否是UTF-8编码*/
int u8_is_utf8_sys() { return 1; }

char *mbs2utf8(const char *s) { return pcs_utils_strdup(s); }

char *utf82mbs(const char *s) { return pcs_utils_strdup(s); }
...
static int get_file_secure_method(const char *src) {
  ...
  if (head.magic != PCS_AES_MAGIC || (head.bits != PCS_SECURE_AES_CBC_128 &&
                                      head.bits != PCS_SECURE_AES_CBC_192 &&
                                      head.bits != PCS_SECURE_AES_CBC_256)) {
    fclose(srcFile);
    return PCS_SECURE_PLAINTEXT;
  }
  fclose(srcFile);
...
static int encrypt_file(const char *src, const char *dst, int secure_method,
                        const char *secure_key, char **pErrMsg) {
  MD5_CTX md5;
  ...
  tmp_local_path =
      (char *)pcs_malloc(strlen(src) + strlen(ENCRYPT_FILE_SUFFIX) + 1);
  ...
  if (rename(tmp_local_path, dst)) {
    fprintf(stderr,
            "Error: The file have been encrypted at %s, but can't rename to "
...
static int decrypt_file(const char *src, const char *dst,
                        const char *secure_key, char **pErrMsg) {
  struct PcsAesHead head = {0};
  MD5_CTX md5;
  unsigned char md5_value[16], buf[PCS_BUFFER_SIZE], out_buf[PCS_BUFFER_SIZE];
  ...
  if (!srcFile) {
    if (pErrMsg) {
      ...
      (*pErrMsg) =
          pcs_utils_sprintf("Error: Can't open the source file: %s\n", src);
    }
    return -1;
  }
  tmp_local_path =
      (char *)pcs_malloc(strlen(src) + strlen(DECRYPT_FILE_SUFFIX) + 1);
  strcpy(tmp_local_path, src);
  ...
  if (file_sz < 32 || ((file_sz - 32) % AES_BLOCK_SIZE) != 0) {
    if (pErrMsg) {
      ...
      (*pErrMsg) = pcs_utils_sprintf(
          "Error: The file is not a encrypt file or is broken.\n");
    }
    fclose(srcFile);
    fclose(dstFile);
    DeleteFileRecursive(tmp_local_path);
  ...
  if (sz != 16) {
    if (pErrMsg) {
      if (*pErrMsg) pcs_free(*pErrMsg);
      (*pErrMsg) =
          pcs_utils_sprintf("Error: Can't read the source file: %s\n", src);
    }
    fclose(srcFile);
    fclose(dstFile);
    DeleteFileRecursive(tmp_local_path);
  ...
  if (head.magic != PCS_AES_MAGIC ||
      (head.bits != 128 && head.bits != 192 && head.bits != 256)) {
    if (pErrMsg) {
      if (*pErrMsg) pcs_free(*pErrMsg);
      (*pErrMsg) = pcs_utils_sprintf(
          "Error: The file is not a encrypt file or is broken.\n");
  ...
  while ((sz = fread(buf, 1, buf_sz, srcFile)) > 0) {
    ...
    if (sz != buf_sz) {
      if (pErrMsg) {
        if (*pErrMsg) pcs_free(*pErrMsg);
        (*pErrMsg) =
            pcs_utils_sprintf("Error: Write data to %s error. \n", dst);
      }
      fclose(srcFile);
      fclose(dstFile);
      DeleteFileRecursive(tmp_local_path);
  ...
  if (rename(tmp_local_path, dst)) {
    if (pErrMsg) {
      if (*pErrMsg) pcs_free(*pErrMsg);
      (*pErrMsg) = pcs_utils_sprintf(
          "Error: The file have been decrypted at %s, but can't rename to "
          "%s.\n You should be rename manual.\n",
          tmp_local_path, dst);
    }
    // DeleteFileRecursive(tmp_local_path);
...
static int encrypt_data(const char *src, size_t src_size, char **dst,
                        size_t *dst_size, int secure_method,
  ...
  for (i = 0; i < blockCount; i++) {
    if ((size_t)((i + 1) * AES_BLOCK_SIZE) > src_size) {
      ...
      memset(buf + src_size - i * AES_BLOCK_SIZE, 0,
             AES_BLOCK_SIZE - (src_size - i * AES_BLOCK_SIZE));
      // encrypt (iv will change)
...
static int decrypt_data(const char *src, size_t src_size, char **dst,
                        size_t *dst_size, const char *secure_key) {
  int rc;
  struct PcsAesHead head = {0};
  MD5_CTX md5;
  AES_KEY aes = {0};
...
static int save_thread_states_to_file(FILE *pf, int64_t offset,
                                      struct DownloadThreadState *state_link) {
...
static int save_upload_thread_states_to_file(
    const char *filename, struct UploadThreadState *state_link) {
  static int magic = THREAD_STATE_MAGIC;
  FILE *pf;
  int rc;
...
/*输出验证码图片，并等待用户输入识别结果*/
static PcsBool verifycode(unsigned char *ptr, size_t size, char *captcha,
                          size_t captchaSize, void *state) {
  static char filename[1024] = {0};
...
static PcsBool input_str(const char *tips, char *value, size_t valueSize,
                         void *state) {
  printf("%s", tips);
...
/*显示上传进度*/
static int upload_progress(void *clientp, double dltotal, double dlnow,
                           double ultotal, double ulnow) {
  char *path = (char *)clientp;
  static char tmp[64];

  tmp[63] = '\0';
  if (path) printf("Upload %s ", path);
...
/*下载文件时，写入文件并显示进度*/
static int download_write(char *ptr, size_t size, size_t contentlength,
                          void *userdata) {
  ...
  ds->speed += size;
  if (ds->cache.total_size >=
      convert_to_real_cache_size(ds->context->cache_size)) {
  ...
  if (tm != ds->time) {
    ...
    ds->time = tm;
    printf("%s", pcs_utils_readable_size(
                     (double)ds->downloaded_size + (double)ds->resume_from, tmp,
                     63, NULL));
    printf("/%s \t",
           pcs_utils_readable_size(
               (double)contentlength + (double)ds->resume_from, tmp, 63, NULL));
    printf("%s/s \t",
           pcs_utils_readable_size((double)ds->speed, tmp, 63, NULL));
    printf("%s            ",
           pcs_utils_readable_left_time(remain_tm, tmp, 63, NULL));
    printf("\r");
    fflush(stdout);
    ds->speed = 0;
  }
...
static int download_write_for_multy_thread(char *ptr, size_t size,
  ...
  if (size > 0) {
    ...
    if (rc) {
      ...
      ds->status =
          DOWNLOAD_STATUS_WRITE_FILE_FAIL; /* Cause all threads to exit. */
      unlock_for_download(ds);
      return 0;
    }
  }
  ...
  if (tm != ds->time) {
    ...
    double percent =
        (ds->file_size > 0)
            ? (double)(100 * (ds->downloaded_size + ds->resume_from)) /
                  (double)ds->file_size
            : 0;
    ds->time = tm;
    printf("\r                                                \r");
    printf("%s", pcs_utils_readable_size(
                     ...
                     63, NULL));
    printf("/%s (%.2f%%)\t",
           pcs_utils_readable_size((double)ds->file_size, tmp, 63, NULL),
           (float)percent);
    printf("%s/s \t",
           pcs_utils_readable_size((double)ds->speed, tmp, 63, NULL));
    printf(" %s        ",
...
/*显示上传进度*/
static int upload_progress_for_multy_thread(void *clientp, double dltotal,
  ...
  if (tm != us->time) {
    ...
    printf("\r                                                \r");
    printf("%s",
           pcs_utils_readable_size((double)us->uploaded_size, tmp, 63, NULL));
    printf("/%s \t",
           pcs_utils_readable_size((double)us->file_size, tmp, 63, NULL));
    printf("%s/s \t",
           pcs_utils_readable_size((double)us->speed, tmp, 63, NULL));
    printf(" %s        ",
           pcs_utils_readable_left_time(remain_tm, tmp, 63, NULL));
    printf("\r");
    fflush(stdout);
...
static const char *size_tostr(size_t size, int *fix_width, char ch) {
  ...
  str[i] = '\0';
  if (*fix_width < 0) *fix_width = (int)i;
  return str;
}
...
static void print_time(const char *format, time_t time) {
  ...
  if (time) tm = localtime(&t);

  if (tm) {
    sprintf(tmp, "%d-%02d-%02d %02d:%02d:%02d", 1900 + tm->tm_year,
...
static void print_filelist_head(int size_width, int md5, int thumb) {
  ...
  putchar('D');
  putchar(' ');
  for (i = 0; i < size_width - 4; i++) putchar(' ');
  printf("Size");
  putchar(' ');
  putchar(' ');
...
/*打印文件列表的数据行*/
static void print_filelist_row(PcsFileInfo *f, int size_width, int md5,
                               int thumb) {
  const char *p;
...
/*打印文件列表*/
static void print_filelist(PcsFileInfoList *list, int *pFileCount,
                           int *pDirCount, int64_t *pTotalSize, int md5,
                           int thumb) {
  char tmp[64] = {0};
  int cnt_file = 0, cnt_dir = 0, size_width = 1, w;
...
static int restore_context(ShellContext *context, const char *filename) {
  ...
  if (item && item->valuestring && item->valuestring[0]) {
    if (!is_absolute_path(item->valuestring)) {
      printf(
          "warning: Invalid context.cookiefile, the value should be absolute "
          "path, use default value: %s.\n",
          context->cookiefile);
    } else {
  ...
  if (item && item->valuestring && item->valuestring[0]) {
    if (!is_absolute_path(item->valuestring)) {
      printf(
          "warning: Invalid context.captchafile, the value should be absolute "
          "path, use default value: %s.\n",
          context->captchafile);
    } else {
  ...
  if (item && item->valuestring && item->valuestring[0]) {
    if (item->valuestring[0] != '/') {
      printf(
          "warning: Invalid context.workdir, the value should be absolute "
          "path, use default value: %s.\n",
          context->workdir);
    } else {
  ...
  if (item) {
    if (((int)item->valueint) < 1) {
      printf(
          "warning: Invalid context.list_page_size, the value should be great "
          "than 0, use default value: %d.\n",
  ...
  if (item && item->valuestring && item->valuestring[0]) {
    if (strcmp(item->valuestring, "name") &&
        strcmp(item->valuestring, "time") &&
        strcmp(item->valuestring, "size")) {
      printf(
          "warning: Invalid context.list_sort_name, the value should be one of "
          "[name|time|size], use default value: %s.\n",
  ...
  if (item && item->valuestring && item->valuestring[0]) {
    if (strcmp(item->valuestring, "asc") && strcmp(item->valuestring, "desc")) {
      printf(
          "warning: Invalid context.list_sort_direction, the value should be "
          "one of [asc|desc], use default value: %s.\n",
  ...
  if (item && item->valuestring && item->valuestring[0]) {
    if (strcmp(item->valuestring, "plaintext") &&
        strcmp(item->valuestring, "aes-cbc-128") &&
        strcmp(item->valuestring, "aes-cbc-192") &&
        strcmp(item->valuestring, "aes-cbc-256")) {
      printf(
          "warning: Invalid context.secure_method, the value should be one of "
          "[plaintext|aes-cbc-128|aes-cbc-192|aes-cbc-256], use default value: "
          "%s.\n",
  ...
  if (item) {
    if (((int)item->valueint) < 1) {
      printf(
          "warning: Invalid context.max_thread, the value should be great than "
          "0, use default value: %d.\n",
          context->max_thread);
    } else {
  ...
  if (item) {
    if (((int)item->valueint) < 0) {
      printf(
          "warning: Invalid context.max_speed_per_thread, the value should be "
          ">= 0, use default value: %d.\n",
  ...
  if (item) {
    if (((int)item->valueint) < 0) {
      printf(
          "warning: Invalid context.max_upload_speed_per_thread, the value "
          "should be >= 0, use default value: %d.\n",
  ...
  if (item) {
    if (((int)item->valueint) < 0) {
      printf(
          "warning: Invalid context.cache_size, the value should be >= 0, use "
          "default value: %d.\n",
          context->cache_size);
    } else {
...
static Pcs *create_pcs(ShellContext *context) {
  ...
  pcs_setopts(pcs, PCS_OPTION_PROGRESS_FUNCTION, (void *)&upload_progress,
              PCS_OPTION_PROGRESS, (void *)((long)PcsFalse), PCS_OPTION_USAGE,
              (void *)context->user_agent,
...
static void destroy_pcs(Pcs *pcs) { pcs_destroy(pcs); }
...
/*打印版本*/
static void version() { printf(program_full_name "%s\n", app_name); }
...
static void usage_compare() {
  ...
  printf("\nDescription:\n");
  printf(
      "  Print the differents between local and net disk. \n"
...
static void usage_download() {
  ...
  printf("\nOptions:\n");
  printf(
      "  -f    Force override the local file when the file exists on local "
      "file system.\n");
  printf("  -h    Print the usage.\n");
  printf("\nSamples:\n");
  printf("  %s download -h\n", app_name);
...
static void usage_fix() {
  ...
  printf(
      "  Some times, we download a file from foreign website, the speed is "
      "slow.\n");
  printf(
      "  In the case, if we know the actual size and md5 of the file, we can "
      "\n");
  ...
  printf("\nOptions:\n");
  printf(
      "  -f    Force override the remote file when the file exists on "
      "netdisk.\n");
  printf("  -h    Print the usage.\n");
  printf("\nSamples:\n");
  printf("  %s fix -h\n", app_name);
  printf(
      "  %s fix 39d768542cd2420771f28b9a3652412f 5849513984 ~/xxx.iso "
      "xxx.iso\n",
      app_name);
}
...
static void usage_login() {
  version();
  printf(
      "\nUsage: %s login [-h] [--username=<user name>] "
      "[--password=<password>]\n",
      app_name);
  printf("\nDescription:\n");
  printf(
      "  The command maybe save captcha image into your local file system, \n"
      "  then you can see the captcha. You can use '%s set "
      "--captcha_file=<path>' \n"
      "  to change the save path, and use '%s context' command to view the "
      "path.\n"
      "  The cookie can reusable, so you can login in your pc, and then upload "
      "the \n"
...
static void usage_set() {
  ...
  printf("\nDescription:\n");
  printf(
      "  Change the context, you can see the context by '%s context' "
      "command.\n",
      app_name);
  printf("\nOptions:\n");
  printf("  -h    Print the usage.\n");
  ...
  printf(
      "  secure_key           String     not null when 'secure_method' is not "
      "'plaintext'\n");
  printf(
      "  secure_method        Enum       "
      "plaintext|aes-cbc-128|aes-cbc-192|aes-cbc-256\n");
  printf("  timeout_retry        Boolean    true|false. \n");
  printf(
      "  max_thread           UInt       > 0 and < %d. The max number of "
      "thread that allow create.\n",
      MAX_THREAD_NUM);
  printf(
      "  max_speed_per_thread Int        >= 0. The max speed in KiB per "
      "thread.\n");
  printf(
      "  max_upload_speed_per_thread Int >= 0. The max speed in KiB per "
      "thread.\n");
  printf("  user_agent           String     set user agent.\n");
  printf(
      "  cache_size           Int        >= 0. The max cache size in KiB.\n");
  printf("\nSamples:\n");
  printf("  %s set -h\n", app_name);
  printf("  %s set --cookie_file=\"/tmp/pcs.cookie\"\n", app_name);
  printf(
      "  %s set --cookie_file=\"/tmp/pcs.cookie\" "
      "--captcha_file=\"/tmp/vc.git\"\n",
      app_name);
  printf(
      "  %s set --list_page_size=20 --list_sort_name=name "
...
static void usage_synch() {
  ...
  printf(
      ...
      "  Default options is '-cdu', means download newer files, upload newer "
      "files \n"
      "  and print confuse files. You can use '-u' to upload newer files only, "
      "use \n"
      "  '-d' to download newer files only, and use '-c' to view confuse "
      "files.\n"
      "  Notes:\n"
...
static void usage() {
  ...
  printf(
      "  set      Change the context, you can print the context by 'context' "
      "command\n");
  printf("  search   Search the files in the specify directory\n");
  printf(
      "  synch    Synch between local and net disk. You can 'compare' "
      "first.\n");
...
static int set_secure_method(ShellContext *context, const char *val) {
  if (!val || !val[0]) return -1;
  if (strcmp(val, "plaintext") && strcmp(val, "aes-cbc-128") &&
      strcmp(val, "aes-cbc-192") && strcmp(val, "aes-cbc-256")) {
    return -1;
  }
...
static int set_max_upload_speed_per_thread(ShellContext *context,
                                           const char *val) {
  const char *p = val;
  int v;
  if (!val || !val[0]) return -1;
...
/*红黑树中用于比较 key 值。当 *a > *b 时，返回 1; 当 *a < *b 时，返回 -1;
 * 当相等时，返回 0。 */
...
static void onGotLocalFile(LocalFileInfo *info, LocalFileInfo *parent,
                           void *state) {
...
static rb_red_blk_tree *meta_load(const char *dir, int recursive) {
  ...
  rb = RBTreeCreate(&rb_compare, &rb_destory_key, &rb_destory_info,
  ...
  if (cnt > 0) putchar('\n');
  if (link) DestroyLocalFileInfoLink(link);
  return rb;
}
...
static inline void decide_op(MyMeta *meta) {
  if ((meta->flag & FLAG_ON_LOCAL) &&
      (meta->flag & FLAG_ON_REMOTE)) { /*文件在本地和网盘中都存在*/
 ...
 *   meta   - 待打印的 meta
 */
static void print_meta_list_row(int first, int second, int other,
                                MyMeta *meta) {
  int i;
  if (first > 0) {
    switch (meta->op_st) {
  ...
  if (meta->flag & FLAG_ON_LOCAL) {
    printf("%s", meta->path);
    i = strlen(meta->path);
    if (meta->local_isdir && i > 0 && meta->path[i - 1] != '/' &&
        meta->path[i - 1] != '\\') {
      putchar('/');
      i++;
    }
  ...
  if (meta->flag & FLAG_ON_REMOTE) {
    ...
    if (meta->remote_isdir && i > 0 && meta->remote_path[i - 1] != '/' &&
        meta->remote_path[i - 1] != '\\') {
      putchar('/');
    }
  }
  putchar('\n');
...
static void print_meta_list_row_err(int first, int second, int other,
                                    MyMeta *meta) {
  int i;
  if (first > 0) {
    switch (meta->op_st) {
...
static void print_meta_list_statistic(struct RBEnumerateState *s,
  ...
  putchar('\n');
  printf(
      "Need Download: %d, Need Upload: %d\n"
      "Confuse: %d, Equal: %d, Other: %d\n"
      "Total: %d",
      s->cnt_left, s->cnt_right, s->cnt_confuse, s->cnt_eq, s->cnt_none,
      s->cnt_total);
  if (print_fail) {
    printf(", Fail: %d\n", s->cnt_fail);
  } else {
...
static int rb_print_meta(void *a, void *state) {
  ...
  } else if (s->page_enable && s->page_size > 0 &&
             (s->printed_count % s->page_size) == 0) {
    s->page_index++;
    printf(
        "\nPrint next page #%d?\n"
        "  yes - Continue print\n"
        ...
        "Press <enter> to continue or input your choice: ",
        s->page_index);
    std_string(tmp, 8);
    if (strlen(tmp) != 0 && pcs_utils_strcmpi(tmp, "y") &&
        pcs_utils_strcmpi(tmp, "yes")) {
      return -1;
    }
    if (strlen(tmp) != 0 &&
        (strcmp(tmp, "Y") == 0 || strcmp(tmp, "YES") == 0)) {
      s->page_enable = 0;
    }
    printf("\n%sPAGE #%d\n", s->prefixion ? s->prefixion : "", s->page_index);
    print_meta_list_head(s->first, s->second, s->other);
  ...
  if (s->process) meta->op_st = OP_ST_PROCESSING;
  ...
  if (meta->op_st == OP_ST_FAIL) s->cnt_fail++;
...
static PcsBool is_login(ShellContext *context, const char *msg) {
  ...
  if (pcsres == PCS_LOGIN) return PcsTrue;
  if (msg) {
    if (msg[0]) printf("%s\n", msg);
  } else if (pcsres == PCS_NOT_LOGIN) {
    printf(
        "You are not logon or your session is time out. You can login by "
        "'login' command.\n");
  } else {
...
static struct DownloadThreadState *pop_download_threadstate(
    struct DownloadState *ds) {
...
#endif
{
  ...
  while (ts) {
    ...
    pcs_setopts(pcs, PCS_OPTION_DOWNLOAD_WRITE_FUNCTION,
                &download_write_for_multy_thread,
                PCS_OPTION_DOWNLOAD_WRITE_FUNCTION_DATA, ts, PCS_OPTION_END);
    res = pcs_download(pcs, ds->remote_file,
                       convert_to_real_speed(context->max_speed_per_thread),
                       ts->postion, ts->end - ts->postion);
    if (res != PCS_OK) {
      int delay;
      lock_for_download(ds);
      if (!ds->pErrMsg) {
...
static void start_download_thread(struct DownloadState *ds, void **pHandle) {
  ...
  pParam, dwFlags, &idThread) WINBASEAPI HANDLE WINAPI
  CreateThread(LPSECURITY_ATTRIBUTES,DWORD,LPTHREAD_START_ROUTINE,PVOID,DWORD,PDWORD);
  第一个参数是指向SECURITY_ATTRIBUTES型态的结构的指针。在Windows
  98中忽略该参数。在Windows NT中，它被设为NULL。
  第二个参数是用于新线程的初始堆栈大小，默认值为0。在任何情况下，Windows根据需要动态延长堆栈的大小。
  第三个参数是指向线程函数的指标。函数名称没有限制，但是必须以下列形式声明:DWORD
  WINAPI ThreadProc (PVOID pParam) ;
  第四个参数为传递给ThreadProc的参数。这样主线程和从属线程就可以共享数据。
  第五个参数通常为0，但当建立的线程不马上执行时为旗标
  第六个参数是一个指针，指向接受执行绪ID值的变量
  */
  thandle =
      CreateThread(NULL, 0, download_thread, (LPVOID)ds, 0, &tid);  // 建立线程
...
static int restore_download_state(struct DownloadState *ds,
                                  const char *tmp_local_path,
                                  int *pendding_count) {
  ...
  if ((tmp_file_size >=
       ds->file_size + 4 + sizeof(struct DownloadThreadState))) {
    FILE *pf;
    int magic;
    int thread_count = 0;
    ...
    while (1) {
      ts = (struct DownloadThreadState *)pcs_malloc(
          sizeof(struct DownloadThreadState));
 ...
 *   pErrMsg        -
 * 如果下载失败时，用于接收失败消息，如果无需失败消息，则传入NULL
 *                    使用完后需调用pcs_free()
 *   op_st          - 用于接收操作状态的。即 OP_ST_FAIL， OP_ST_SUCC，
 * OP_ST_SKIP 成功后返回0，失败后返回非0值
 */
static inline int do_download(ShellContext *context, const char *local_file,
                              const char *remote_file, time_t remote_mtime,
                              const char *local_basedir,
                              const char *remote_basedir, const char *md5,
                              char **pErrMsg, int *op_st) {
  PcsRes res;
  struct DownloadState ds = {0};
  char *local_path, *remote_path, *dir, *tmp_local_path;
  ...
  tmp_local_path =
      (char *)pcs_malloc(strlen(local_path) + (md5 ? strlen(md5) + 1 : 0) +
                         strlen(TEMP_FILE_SUFFIX) + 1);
  ...
  if (fsize <= MIN_SLICE_SIZE) {
    ...
    if (!ds.pf) {
      if (pErrMsg) {
        if (*pErrMsg) pcs_free(*pErrMsg);
        (*pErrMsg) = pcs_utils_sprintf(
            "Can't open or create the temp file: %s\n", tmp_local_path);
      }
      if (op_st) (*op_st) = OP_ST_FAIL;
      DeleteFileRecursive(tmp_local_path);
      pcs_free(tmp_local_path);
    ...
    ds.cache.fp = ds.pf;
    pcs_setopts(context->pcs, PCS_OPTION_DOWNLOAD_WRITE_FUNCTION,
                &download_write, PCS_OPTION_DOWNLOAD_WRITE_FUNCTION_DATA, &ds,
                PCS_OPTION_END);
    res = pcs_download(context->pcs, remote_path,
                       convert_to_real_speed(context->max_speed_per_thread), 0,
                       0);
    ...
    curl_off_t start = 0;
    const char *mode = "rb+";
    int slice_count, pendding_slice_count = 0, thread_count,
                     running_thread_count, i, is_success;
    int64_t slice_size;
#ifdef _WIN32
    HANDLE *handles = NULL;
#endif
    if (restore_download_state(&ds, tmp_local_path, &pendding_slice_count)) {
      ...
      // ds.file_size + 4 + sizeof(struct DownloadThreadState) * slice_count,
      // slice_size, slice_count);
      if (create_file(tmp_local_path,
                      ds.file_size + 4 +
                          sizeof(struct DownloadThreadState) * slice_count)) {
        if (pErrMsg) {
          if (*pErrMsg) pcs_free(*pErrMsg);
          (*pErrMsg) = pcs_utils_sprintf(
              "Can't create the temp file: %s, maybe have no disk space.\n",
              tmp_local_path);
        }
        if (op_st) (*op_st) = OP_ST_FAIL;
        DeleteFileRecursive(tmp_local_path);
    ...
    if (save_thread_states_to_file(ds.pf, ds.file_size, ds.threads)) {
      if (pErrMsg) {
        if (*pErrMsg) pcs_free(*pErrMsg);
        (*pErrMsg) = pcs_utils_sprintf(
            "Can't save slices into temporary file: %s \n", tmp_local_path);
      }
      if (op_st) (*op_st) = OP_ST_FAIL;
      DeleteFileRecursive(tmp_local_path);
      pcs_free(tmp_local_path);
    ...
    if (!is_success) {
      if (pErrMsg) {
        if (!(*pErrMsg)) (*pErrMsg) = pcs_utils_sprintf("Download fail.\n");
      }
      if (op_st) (*op_st) = OP_ST_FAIL;
      DeleteFileRecursive(tmp_local_path);
      pcs_free(tmp_local_path);
  ...
  if (secure_method == PCS_SECURE_AES_CBC_128 ||
    ...
    if (decrypt_file(tmp_local_path, local_path, context->secure_key,
                     pErrMsg)) {
      // if (pErrMsg) {
      //	if (*pErrMsg) pcs_free(*pErrMsg);
      //	(*pErrMsg) = pcs_utils_sprintf("Error: The file have been
      // download at %s, but can't decrypted to %s.\n" 		" Maybe broken
      // data.", tmp_local_path, local_path);
      //}
      if (pErrMsg) {
        if (!(*pErrMsg))
    ...
    if (pErrMsg) {
      if (*pErrMsg) pcs_free(*pErrMsg);
      (*pErrMsg) = pcs_utils_sprintf(
          "The file have been download at %s, but can't rename to %s.\n"
          " You should be rename manual.",
...
static struct UploadThreadState *pop_upload_threadstate(
    struct UploadState *us) {
...
static size_t read_slice(void *ptr, size_t size, size_t nmemb, void *userdata) {
  ...
  // printf("%llu - %llu (%llu) \n", (long long)ts->start + ts->uploaded_size,
  // (long long)ts->start + ts->uploaded_size + sz, (long long)sz);
  us->speed += sz;
  us->uploaded_size += sz;
  ts->uploaded_size += sz;
...
#endif
{
  ...
  while (ts) {
    ...
    res = pcs_upload_slicefile(
        pcs, &read_slice, ts, (size_t)(ts->end - ts->start),
...
#ifdef _WIN32
      printf(
          "Upload slice failed, retry delay 10 second, tid: %x. message: %s\n",
          GetCurrentThreadId(), pcs_strerror(pcs));
#else
      printf(
          "Upload slice failed, retry delay 10 second, tid: %p. message: %s\n",
...
static void start_upload_thread(struct UploadState *us, void **pHandle) {
  ...
  */
  thandle =
      CreateThread(NULL, 0, upload_thread, (LPVOID)us, 0, &tid);  // 建立线程
  if (pHandle) *pHandle = thandle;
  if (!thandle) {
    printf("Error: Can't create download thread.\n");
    lock_for_upload(us);
...
static int restore_upload_state(struct UploadState *us,
                                const char *slice_local_path,
                                int *pendding_count) {
  LocalFileInfo *tmpFileInfo;
  ...
  us->uploaded_size = 0;
  while (1) {
    ts = (struct UploadThreadState *)pcs_malloc(
        sizeof(struct UploadThreadState));
...
static inline int do_upload(ShellContext *context, const char *local_file,
                            const char *remote_file, PcsBool is_force,
                            const char *local_basedir,
                            const char *remote_basedir, char **pErrMsg,
                            int *op_st) {
  ...
  if (context->secure_enable) {
    ...
    if (context_secure_method == PCS_SECURE_AES_CBC_128 ||
      ...
      if (secure_method != PCS_SECURE_AES_CBC_128 &&
          secure_method != PCS_SECURE_AES_CBC_192 &&
          secure_method != PCS_SECURE_AES_CBC_256) {
        char *tmp_local_path = (char *)pcs_malloc(strlen(local_path) +
                                                  strlen(TEMP_FILE_SUFFIX) + 1);
        strcpy(tmp_local_path, local_path);
        strcat(tmp_local_path, TEMP_FILE_SUFFIX);
        if (encrypt_file(local_path, tmp_local_path, context_secure_method,
                         context->secure_key, pErrMsg)) {
          // if (pErrMsg) {
          //	if (*pErrMsg) pcs_free(*pErrMsg);
          //	(*pErrMsg) = pcs_utils_sprintf("Error: can't encrypt the file
          //%s.\n", 		local_path);
          //}
          if (op_st) (*op_st) = OP_ST_FAIL;
  ...
  if (content_length < 0) {
    if (pErrMsg) {
      if (*pErrMsg) pcs_free(*pErrMsg);
      (*pErrMsg) = pcs_utils_sprintf("%s. local_path=%s, remote_path=%s\n",
                                     pcs_strerror(context->pcs), local_path,
                                     remote_path);
    }
    if (op_st) (*op_st) = OP_ST_FAIL;
  ...
  if (!res && content_length <= MIN_UPLOAD_SLICE_SIZE) {
    pcs_setopts(context->pcs, PCS_OPTION_PROGRESS_FUNCTION, &upload_progress,
                PCS_OPTION_PROGRESS_FUNCTION_DATE, NULL, PCS_OPTION_PROGRESS,
                (void *)((long)PcsTrue),
                // PCS_OPTION_TIMEOUT, (void *)0L,
                PCS_OPTION_END);
    res =
        pcs_upload(context->pcs, remote_path, is_force, local_path,
                   convert_to_real_speed(context->max_upload_speed_per_thread));
    ...
    slice_file = (char *)pcs_malloc(strlen(local_path) + 33 +
                                    strlen(SLICE_FILE_SUFFIX) + 1);
    ...
    if (restore_upload_state(&us, slice_file, &pendding_slice_count)) {
      ...
      if ((content_length % slice_count)) slice_size++;
    ...
    if (save_upload_thread_states_to_file(slice_file, us.threads)) {
      if (pErrMsg) {
        if (*pErrMsg) pcs_free(*pErrMsg);
        (*pErrMsg) =
            pcs_utils_sprintf("Can't save slices into file: %s \n", slice_file);
      }
      if (op_st) (*op_st) = OP_ST_FAIL;
      if (del_local_file) DeleteFileRecursive(local_path);
    ...
    if (!is_success) {
      if (pErrMsg) {
        if (!(*pErrMsg)) (*pErrMsg) = pcs_utils_sprintf("Upload fail.\n");
      }
      if (op_st) (*op_st) = OP_ST_FAIL;
      if (del_local_file) DeleteFileRecursive(local_path);
      pcs_free(local_path);
  ...
  if (res) {
    ...
    if (diskName && orgName && orgName[0] == '.' && diskName[0] != '.') {
      PcsPanApiRes *res2;
      PcsSList2 sl = {res->path, orgName, NULL};
      PcsSList sl2 = {res->path, NULL};
      pcs_free(orgName);
          ...
          if (is_force) {
            ...
            if (!res2 || res2->error != 0) {
              if (pErrMsg) {
                if (*pErrMsg) pcs_free(*pErrMsg);
                (*pErrMsg) = pcs_utils_sprintf(
                    "Error: Can't delete the %s, so can't rename %s to %s. You "
                    "can rename manually.\n",
            ...
            if (pErrMsg) {
              if (*pErrMsg) pcs_free(*pErrMsg);
              (*pErrMsg) = pcs_utils_sprintf(
                  "Error: Can't rename %s to %s. You can rename manually.\n",
                  sl.string1, sl.string2);
            }
            break;
          }
...
static int cmd_cd(ShellContext *context, struct args *arg) {
  ...
  if (strcmp(p, "/")) {
    ...
    if (!meta) {
      fprintf(stderr,
              "Error: The target directory not exist, or have error: %s\n",
              pcs_strerror(context->pcs));
      pcs_free(p);
      return -1;
    }
...
static int cmd_copy(ShellContext *context, struct args *arg) {
  ...
  slist.string1 =
      combin_net_disk_path(context->workdir, arg->argv[0]); /* path */
  slist.string2 =
      combin_net_disk_path(context->workdir, arg->argv[1]); /* new_name */
  slist.next = NULL;
...
struct compare_arg {
  ...
  /*当State准备好后调用一次本方法*/
  void (*onRBEnumerateStatePrepared)(ShellContext *context, compare_arg *arg,
                                     rb_red_blk_tree *rb,
                                     struct RBEnumerateState *state, void *st);
};
 ...
 *   skip        - 目录跳过的字节数。
 */
static MyMeta *compare_file(const LocalFileInfo *local,
                            const PcsFileInfo *remote) {
  MyMeta *meta = NULL;
 ...
 * 成功则返回0；否则返回非0值
 */
static int combin_with_remote_dir_files(ShellContext *context,
                                        rb_red_blk_tree *rb,
                                        const char *remote_dir, int recursive,
                                        int skip, int *total_cnt,
                                        int check_local_dir_exist) {
  PcsFileInfoList *list = NULL;
  PcsFileInfoListIterater iterater;
  PcsFileInfo *info = NULL;
  int page_index = 1, page_size = 1000;
  int cnt = 0, second;
  rb_red_blk_node *rbn;
  MyMeta *meta;

  while (1) {
    list = pcs_list(context->pcs, remote_dir, page_index, page_size, "name",
                    PcsFalse);
    if (!list) {
      if (pcs_strerror(context->pcs)) {
        fprintf(stderr, "Error: %s \n", pcs_strerror(context->pcs));
        if (context->timeout_retry &&
            strstr(pcs_strerror(context->pcs),
                   "Can't get response from the remote server") >= 0) {
          second = 10;
          while (second > 0) {
        ...
        if (info->isdir) {
          if (check_local_dir_exist) {
            ...
            if (rbn && ((meta = (MyMeta *)rbn->info)->flag & FLAG_ON_LOCAL)) {
              if (combin_with_remote_dir_files(context, rb, info->path,
                                               recursive, skip, total_cnt,
                                               check_local_dir_exist)) {
                pcs_filist_destroy(list);
                return -1;
              }
            }
          } else if (combin_with_remote_dir_files(context, rb, info->path,
                                                  recursive, skip, total_cnt,
                                                  check_local_dir_exist)) {
            pcs_filist_destroy(list);
            return -1;
...
static int on_compared_file(ShellContext *context, compare_arg *arg, MyMeta *mm,
                            void *state) {
  int first, second, other;
  first = 0;
...
static int on_compared_dir(ShellContext *context, compare_arg *arg,
                           rb_red_blk_tree *rb, void *st) {
  struct RBEnumerateState state = {0};
  int printed_count = 0;
  if (!arg->print_eq && !arg->print_left && !arg->print_right &&
      !arg->print_confuse) {
  ...
  if (arg->onRBEnumerateStatePrepared) {
    ...
    printf(
        "\nPrint Download: %s, Print Upload: %s, Print Confuse: %s, Print "
        "Equal: %s\n",
        arg->print_left ? "on" : "off", arg->print_right ? "on" : "off",
        arg->print_confuse ? "on" : "off", arg->print_eq ? "on" : "off");
  }
  ...
  if (state.cnt_confuse > 0 && arg->print_confuse &&
      !(state.print_op & OP_CONFUSE)) {
    // printf("\nwarning: There are number of confuse items. The confuse means
    // that don't know how to process.\n");
    state.print_op = OP_CONFUSE;
    state.page_index = 1;
...
static int compare(ShellContext *context, compare_arg *arg,
                   int (*onComparedFile)(ShellContext *context,
                                         compare_arg *arg, MyMeta *mm,
                                         void *state),
                   void *comparedFileState,
                   int (*onComparedDir)(ShellContext *context, compare_arg *arg,
                                        rb_red_blk_tree *rb, void *state),
  ...
  if (local->isdir && remote->isdir) {
    ...
    if (combin_with_remote_dir_files(context, rb, remote->path, arg->recursive,
                                     skip, &total_cnt,
                                     arg->check_local_dir_exist)) {
...
static int cmd_compare(ShellContext *context, struct args *arg) {
  ...
  int rc = compare(context, &cmpArg, &on_compared_file, NULL, &on_compared_dir,
                   NULL);
  if (!u8_is_utf8_sys()) {
...
static int cmd_download(ShellContext *context, struct args *arg) {
  ...
  if (local && local->isdir) {
    ...
    if (!is_force) {
      fprintf(stderr,
              "Error: The local file: %s exist. You can specify '-f' to force "
              "override.\n",
              arg->argv[1]);
      DestroyLocalFileInfo(local);
      if (!u8_is_utf8_sys()) pcs_free(locPath);
  ...
  if (strcmp(path, "/") == 0) {
    fprintf(stderr,
            "Error: Can't download root directory. \nYou can use 'synch -cd' "
  ...
  if (meta->isdir) {
    fprintf(stderr,
            "Error: The remote file is directory. \nYou can use 'synch -cd' "
            "command to download the directory.\n");
    pcs_fileinfo_destroy(meta);
    pcs_free(path);
    if (!u8_is_utf8_sys()) pcs_free(locPath);
  ...
  if (do_download(context, locPath, meta->path, meta->server_mtime, "",
                  context->workdir, meta->md5, &errmsg, NULL)) {
    fprintf(stderr, "Error: %s\n", errmsg);
    pcs_fileinfo_destroy(meta);
...
static int cmd_echo(ShellContext *context, struct args *arg) {
  ...
  if (strcmp(path, "/") == 0) {
    fprintf(stderr,
            "Error: Can't specify root directory, you should specify the file "
            "name.\n");
    pcs_free(path);
    return -1;
  }
  ...
  if (context->secure_enable) {
    ...
    if (context_secure_method == PCS_SECURE_AES_CBC_128 ||
        context_secure_method == PCS_SECURE_AES_CBC_192 ||
        context_secure_method == PCS_SECURE_AES_CBC_256) {
      if (encrypt_data(t, sz, &buf, &bufsz, context_secure_method,
                       context->secure_key)) {
  ...
  f = pcs_upload_buffer(
      context->pcs, path, PcsTrue, buf, sz,
      convert_to_real_speed(context->max_upload_speed_per_thread));
  if (!f) {
    fprintf(stderr, "Error: %s. path = %s\n", pcs_strerror(context->pcs), path);
    pcs_free(path);
...
static int cmd_encode(ShellContext *context, struct args *arg) {
  ...
  if (has_opt(arg, "d")) decrypt = 1;
  if (has_opt(arg, "e")) encrypt = 1;
  if (has_opt(arg, "f")) force = 1;
  ...
  if (dstInfo) {
    if (dstInfo->isdir) {
      ...
      fprintf(
          stderr,
          "Error: The dest is exist. You can use '-f' to force override. %s",
          arg->argv[1]);
      DestroyLocalFileInfo(dstInfo);
      if (!u8_is_utf8_sys()) {
        pcs_free(src);
  ...
  if (encrypt) {
    ...
    if (secure_method != PCS_SECURE_AES_CBC_128 &&
        ...
        secure_method != PCS_SECURE_AES_CBC_256) {
      fprintf(stderr,
              "Error: You have not set the encrypt method, the method should "
              "be one of [aes-cbc-128|aes-cbc-192|aes-cbc-256]. You can set it "
              "by '%s set'.\n",
              app_name);
      if (!u8_is_utf8_sys()) {
        pcs_free(src);
    ...
    if (!context->secure_key || strlen(context->secure_key) == 0) {
      fprintf(stderr,
              "Error: You have not set the encrypt key. You can set it by '%s "
              "set'.",
              app_name);
      if (!u8_is_utf8_sys()) {
        pcs_free(src);
...
static int cmd_fix(ShellContext *context, struct args *arg) {
  ...
  if (local->size < PCS_RAPIDUPLOAD_THRESHOLD) {
    ...
    if (!u8_is_utf8_sys()) pcs_free(scrapFile);
    fprintf(stderr,
            "The scrap size is not satisfied, the file must be great than %d.",
            PCS_RAPIDUPLOAD_THRESHOLD);
    return -1;
  }
  /*检查本地文件 - 结束*/
  ...
  if (meta && meta->isdir) {
    ...
    fprintf(stderr,
            "Error: The remote file exist. You can specify '-f' to force "
            "override. %s\n",
            path);
    pcs_fileinfo_destroy(meta);
    pcs_free(path);
  ...
  if (!pcs_md5_file_slice(context->pcs, scrapFile, 0, PCS_RAPIDUPLOAD_THRESHOLD,
                          slice_md5)) {
  ...
  /*开始修复*/
  meta = pcs_rapid_upload_r(context->pcs, path, is_force ? PcsTrue : PcsFalse,
...
static int cmd_list(ShellContext *context, struct args *arg) {
  ...
  while (1) {
    list = pcs_list(
        context->pcs, path, page_index, context->list_page_size,
        context->list_sort_name,
  ...
  if (page_index > 1) {
    ...
    tmp[63] = '\0';
    printf(
        "Total Page: %d, Total Size: %s, File Count: %d, Directory Count: %d\n",
...
static int cmd_login(ShellContext *context, struct args *arg) {
  ...
  if (is_login(context, "")) {
    printf(
        "You have been logon, you can use 'who' command to print the UID. \n"
...
static int cmd_logout(ShellContext *context, struct args *arg) {
  ...
  if (!is_login(context, "")) {
    ...
    fprintf(stderr,
            "You are not login, you can use 'login' command to login.\n");
    return -1;
  }
...
static int cmd_set(ShellContext *context, struct args *arg) {
  char *val = NULL;
  int count = 0;
  if (test_arg(arg, 0, 0, "cookie_file", "captcha_file", "list_page_size",
               "list_sort_name", "list_sort_direction", "secure_method",
               "secure_key", "secure_enable", "timeout_retry", "max_thread",
               "max_speed_per_thread", "user-agent", "cache_size", "h", "help",
               NULL) &&
      arg->optc == 0) {
    usage_set();
    return -1;
...
static int cmd_search(ShellContext *context, struct args *arg) {
  ...
  list = pcs_search(context->pcs, path, keyword,
                    is_recursive ? PcsTrue : PcsFalse);
  if (!list) {
    if (pcs_strerror(context->pcs)) {
      fprintf(stderr, "Error: %s\n", pcs_strerror(context->pcs));
      pcs_free(path);
...
static int synchDownload(MyMeta *meta, struct RBEnumerateState *s,
                         void *state) {
  ...
  if (meta->remote_isdir) { /*跳过目录*/
    ...
    if (CreateDirectoryRecursive(local_path) != MKDIR_OK) {
      if (meta->msg) pcs_free(meta->msg);
      meta->msg = pcs_utils_sprintf("Error: Can't create the directory: %s",
                                    local_path);
  ...
  return do_download(s->context, meta->path, meta->remote_path,
                     meta->remote_mtime, s->local_basedir, s->remote_basedir,
                     meta->md5, &meta->msg, &meta->op_st);
}
...
static int synchUpload(MyMeta *meta, struct RBEnumerateState *s, void *state) {
  ...
  if (meta->local_isdir) { /*跳过目录*/
    if (meta->local_filecount == 0 &&
        !(meta->flag & FLAG_ON_REMOTE)) { /*其下没有文件，因此需手动创建目录*/
      /*创建目录*/
      ...
      if (res != PCS_OK) {
        if (meta->msg) pcs_free(meta->msg);
        meta->msg =
            pcs_utils_sprintf("Error: Can't create the directory %s: %s",
                              remote_path, pcs_strerror(s->context->pcs));
        pcs_free(remote_path);
        meta->op_st = OP_ST_FAIL;
        return -1;
      }
  ...
  return do_upload(
      s->context, meta->path,
      (meta->flag & FLAG_ON_REMOTE) ? meta->remote_path : meta->path, PcsTrue,
      s->local_basedir, s->remote_basedir, &meta->msg, &meta->op_st);
}

static int synchOnPrepare(MyMeta *meta, struct RBEnumerateState *s,
                          void *state) {
  if (meta->msg) {
    pcs_free(meta->msg);
    meta->msg = NULL;
...
static void synchOnRBEnumStatePrepared(ShellContext *context, compare_arg *arg,
                                       rb_red_blk_tree *rb,
                                       struct RBEnumerateState *state,
                                       void *st) {
  /*state->print_op &= OP_NONE;
  if (!arg->print_eq && !arg->print_left && !arg->print_right &&
  !arg->print_confuse) state->print_op = OP_RIGHT | OP_CONFUSE; if
  (arg->print_eq) state->print_op |= OP_EQ; if (arg->print_left) state->print_op
  |= OP_LEFT; if (arg->print_right) state->print_op |= OP_RIGHT; if
  (arg->print_confuse) state->print_op |= OP_CONFUSE;*/
  state->page_size = 0;
  state->page_index = 0;
  state->first = 6;
...
static int synchFile(ShellContext *context, compare_arg *arg, MyMeta *meta,
                     void *state) {
  int first, second, other;
  if (meta->msg) {
    pcs_free(meta->msg);
  ...
  switch (meta->op) {
    case OP_LEFT: {
      if (arg->dry_run)
        ...
        do_download(context, meta->path, meta->remote_path, meta->remote_mtime,
                    arg->local_file, arg->remote_file, meta->md5, &meta->msg,
                    &meta->op_st);
      break;
    }
    case OP_RIGHT: {
      if (arg->dry_run)
        ...
        do_upload(
            context, meta->path,
            (meta->flag & FLAG_ON_REMOTE) ? meta->remote_path : meta->path,
            PcsTrue, arg->local_file, arg->remote_file, &meta->msg,
            &meta->op_st);
      break;
    }
    case OP_EQ:
...
static int cmd_upload(ShellContext *context, struct args *arg) {
  ...
  if (!local) {
    ...
    fprintf(stderr,
            "Error: The local file is directory: %s. \nYou can use 'synch -cu' "
            "command to upload the directory.\n",
            arg->argv[0]);
    DestroyLocalFileInfo(local);
    if (!u8_is_utf8_sys()) pcs_free(locPath);
    return -1;
  ...
  if (meta && meta->isdir) {
    ...
    if (diskName[0] ==
      ...
      PcsPanApiRes *res2;
      PcsSList sl2 = {meta->path, NULL};
      res2 = pcs_delete(context->pcs, &sl2);
      if (!res2 || res2->error != 0) {
        fprintf(stderr,
                "Error: Can't delete the %s, so can't override the file.\n",
                sl2.string);
        if (res2) {
          pcs_pan_api_res_destroy(res2);
          res2 = NULL;
  ...
  /*开始上传*/
  if (do_upload(context, locPath, path, is_force ? PcsTrue : PcsFalse, "",
                context->workdir, &errmsg, NULL)) {
    fprintf(stderr, "Error: %s\n", errmsg);
    if (errmsg) pcs_free(errmsg);
    pcs_free(path);
    if (meta) pcs_fileinfo_destroy(meta);
...
static int exec_cmd(ShellContext *context, struct args *arg) {
  ...
  if (!arg->cmd) {
    if (has_opt(arg, "v") || has_opt(arg, "version") ||
        has_opt(arg, "version")) {
