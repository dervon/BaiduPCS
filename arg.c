...
int parse_arg(struct args *arg, int argc, char *argv[],
  ...
  if (arg->optc > 0)
    arg->opts = ht_create((int)((float)arg->optc * HASH_EXTEND_MULTIPLIER), 0,
                          &free_opt);
  if (arg->argc > 0)
  ...
  for (i = 1; i < argc; ++i) {
    ...
    if (*p == '-') {
      ...
      if (*p == '-') {
        ...
        if (ht_add(arg->opts, p, val - p,
                   (*val) == '='
                       ? (s2utf8 ? s2utf8(val + 1) : arg_strdup(val + 1))
                       : NULL))
          return -1; /*添加到哈希表中失败*/
      } else {
        op = p;
        while (*op) {
          ...
          if (ht_add(arg->opts, op, 1, NULL)) return -1; /*添加到哈希表中失败*/
          op++;
        }
      }
    } else if (!arg->cmd) {
