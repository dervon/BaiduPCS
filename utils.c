...
/*
 * 判断arr数组中是否存在字符串str，如果存在则返回其标号（标号为 [索引] +
 * 1），否则返回0。 比较时区分大小写。 arr  -
 * 存储很多字符串的数组，数组最后一个元素必须为NULL。 str  -
 * 判断是否存在的字符串 len  - 字符串长度。 如果传入-1，则'\0'作为其结束标记。
 */
...
#ifdef WIN32
  char buf[MAX_PATH] = "";
  char *p = i_strdup(base, basesz), *f = i_strdup(filename, -1);
  p = fix_win_path(p);
  f = fix_win_path(f);
    ...
    if ((((int)content[1]) & 0xBB) == 0xBB) {
      if ((((int)content[2]) & 0xBF) == 0xBF) {
        ...
        printf("Wrong UTF-8 BOM: BOM=0x%2X%2X%2X %s\n", content[0] & 0xFF,
               content[1] & 0xFF, content[2] & 0xFF, file);
        return -1;
      }
    } else {
      printf("Wrong UTF-8 BOM: BOM=0x%2X%2X%2X %s\n", content[0] & 0xFF,
...
char *time2str(char *buf, const time_t *t) {
  ...
  snprintf(chTmp, sizeof(chTmp), "%04d-%02d-%02d %02d:%02d:%02d", p->tm_year,
