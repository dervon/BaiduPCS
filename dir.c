...
static char *path_dup(const char *path, const char **pName) {
  ...
  p += len - 1;
  if (*p == '/' || *p == '\\')
    *p = '\0'; /*如果最后一个字符是'/'或者'\\'的话，则清掉*/
  if (pName) {
    while (p > result && *p != '/' && *p != '\\')
      p--; /*找到第一个'/'或'\\'的地方，用于设定pName值*/
    if (*p == '/' || *p == '\\') {
      p++;
      (*pName) = p;
    } else {
...
static LocalFileInfo *CreateLocalFileInfo(const char *parentPath,
                                          const char *filename, int isdir,
...
LocalFileInfo *GetLocalFileInfo(const char *file) {
  ...
  if (hFind != INVALID_HANDLE_VALUE) {
    if (fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) /*为目录*/
      info = CreateLocalFileInfo(
          file, NULL, 1, FileTimeToTime_t(fd.ftLastWriteTime, NULL), 0, NULL);
    else { /*为文件*/
      int64_t fsize = fd.nFileSizeHigh;
      fsize <<= 32;
      fsize |= fd.nFileSizeLow;
      info = CreateLocalFileInfo(file, NULL, 0,
                                 FileTimeToTime_t(fd.ftLastWriteTime, NULL),
                                 fsize, NULL);
    }
  }
#else
...
static int GetDirectoryFilesHelp(const char *dir, LocalFileInfo **pCusor,
                                 int recursive, int skip, int *pCnt,
                                 LocalFileInfo *parent,
                                 void (*on)(LocalFileInfo *info,
                                            LocalFileInfo *parent, void *state),
                                 void *state) {
#ifdef WIN32
  ...
  while (!(done = _findnext(handle, &filefind))) {
    if (!strcmp(filefind.name, ".") || !strcmp(filefind.name, "..")) continue;
    if ((_A_SUBDIR == filefind.attrib)) {
      info = CreateLocalFileInfo(dir + skip, filefind.name, 1,
                                 filefind.time_write, 0, parent);
      cusor->next = info;
      cusor = info;
      if (pCnt) (*pCnt)++;
      ...
      if (recursive) {
        ...
        strcpy(subdir, dir);
        if (subdir[len - 1] != '/' && subdir[len - 1] != '\\')
          strcat(subdir, "\\");
        strcat(subdir, filefind.name);
        if (GetDirectoryFilesHelp(subdir, &cusor, recursive, skip, pCnt, info,
                                  on, state)) {
          _findclose(handle);
          *pCusor = cusor;
          pcs_free(subdir);
      ...
      info = CreateLocalFileInfo(dir + skip, filefind.name, 0,
                                 filefind.time_write, filefind.size, parent);
      cusor->next = info;
      cusor = info;
      if (pCnt) (*pCnt)++;
      if (on) (*on)(info, parent, state);
  ...
  *pCusor = cusor;
  ...
  while ((ent = readdir(pDir)) != NULL) {
    ...
    ent->d_type =
        S_ISREG(path_stat.st_mode) ? 8 : (S_ISDIR(path_stat.st_mode) ? 4 : 0);
    pcs_free(subdir);
    if (ent->d_type == 4) {
      ...
      if (subdir[len - 1] != '/' && subdir[len - 1] != '\\')
        strcat(subdir, "/");
      strcat(subdir, ent->d_name);
      if (!stat(subdir, &st)) info->mtime = st.st_mtime;
      if (pCnt) (*pCnt)++;
      if (on) (*on)(info, parent, state);
      if (recursive) {
        if (GetDirectoryFilesHelp(subdir, &cusor, recursive, skip, pCnt, info,
...
int GetDirectoryFiles(LocalFileInfo **pLink, const char *dir, int recursive,
                      void (*on)(LocalFileInfo *info, LocalFileInfo *parent,
                                 void *state),
                      void *state) {
  ...
  if (GetDirectoryFilesHelp(dir, &cusor, recursive, skip, &cnt, NULL, on,
                            state)) {
...
int CreateDirectoryRecursive(const char *path) {
  ...
  if (!path || (len = strlen(path)) == 0) return MKDIR_OK;
#ifdef _WIN32
  if (len == 2 && *(path + 1) == ':' &&
